...
const SENSOR_CLI = path.join(REPO_ROOT, 'sensor', 'dist', 'bin', 'lattice-sensor.js');
...
test(
  ...
  async (t) => {
    ...
    assert.equal(initResp.result.serverInfo.version, '0.7.2-lattice.1');
    ...
    assert.match(text, /\*\*Lattice sensor version:\*\* 0\.7\.2-lattice\.1/);
...
test(
  ...
  async (t) => {
    ...
    assert.equal(lock.version, '0.7.2-lattice.1');
