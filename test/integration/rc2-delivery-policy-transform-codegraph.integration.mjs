...
test('accepted registry shardはfresh Codegraphで3 resolverへのexact calleeを持つ', async (context) => {
  const repoRoot = await mkdtemp(path.join(tmpdir(), 'lattice-rc2-codegraph-transform-'));
  ...
  const querySet = {
    schema: 'lattice.rc2.codegraph_observability_query_set.v1',
    queries: [
  ...
  const evidence = await collectCodegraphEvidence({ cwd: repoRoot, querySet });
