...
import {
  TodoStoreError, appendImportedPlan, appendTodoEvent, applyTodoRevision, buildTodoPlan,
...
async function carriedImportedDoneFixture(context) {
  ...
  const source = (origin_line) => ({ schema: 'lattice.todo_import_source.v1',
    origin_plan_ref: 'plan.md', origin_line, source_commit: sourceCommit });
...
test('完了時刻が判明しているcarried doneはpromoteを拒否し続ける', async (t) => {
  const { root } = await carriedImportedDoneFixture(t);
  // A1はcompleted_atが確定しているためpromotionの対象ではない。
  await assert.rejects(
    appendTodoEvent({ repoRoot: root,
      writer: createTodoStoreWriter({ caller: 'g5-authoring' }), planKey: 'archive', now: NOW,
      event: { kind: 'done', task_id: 'A1', actor: ACTOR, recorded_at: NOW,
        payload: { done_mode: 'evidence_promotion', imported: true,
          evidence: promotionEvidence(root) } } }),
    (error) => error instanceof TodoStoreError
      && error.detail.reason === 'invalid_evidence_promotion',
  );
});
