...
export const TODO_GANTT_RENDERER_VERSION = 'lattice.todo_gantt_renderer.v4';
...
export function renderTodoGanttHtml({
  ...
  const html = `<!doctype html><html lang="ja"><head><meta http-equiv="Content-Security-Policy" content="default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Lattice Todo Gantt</title><style>${CSS}</style></head><body data-gantt-root data-view-state="all"><main class="shell"><section class="gantt-pane" aria-label="依存工程図"><div class="diagram-toolbar" role="group" aria-label="図のズーム"><button type="button" data-zoom-action="out" aria-label="縮小">−</button><button type="button" data-zoom-action="reset">100%</button><button type="button" data-zoom-action="in" aria-label="拡大">＋</button><button type="button" data-zoom-action="fit">全体表示</button><output class="zoom-readout" data-zoom-output aria-live="polite">100%</output><span class="diagram-note">最長依存鎖はunit-weightの構造深さであり実時間・資源律速ではない。</span></div><div class="diagram-scroll" data-diagram-scroll tabindex="0" aria-label="縦方向を主にスクロール可能な依存工程図">${svg}</div></section><aside class="narrative-pane" aria-label="plan Markdown文書"><div class="toolbar"><button type="button" data-show-all>全文表示へ戻る</button></div><div class="narrative-document">${documents}</div></aside></main><script type="application/json" id="todo-gantt-data">${staticData}</script><script>${CONTROLLER}</script></body></html>`;
