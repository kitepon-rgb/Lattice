...
export function verifyReleaseCommit(directory = projectDirectory) {
  const git = (...args) => {
  ...
  // 成果物が公開される。untrackedも同じ経路でpayloadへ入る（0.12.10のpack試行で、
  // 未commitのsrc/todo-gantt-scope.mjsが実際にtarballへ混入した）ため、tracked変更と
  // 同じく拒否する。ignore済みのdist/やbuild/はgit statusに現れないので対象外のまま。
  const dirty = git('status', '--porcelain', '--untracked-files=normal');
  assert.equal(
    dirty.stdout,
    '',
    'working treeにcommitされていない内容があります。publish対象commitとpayloadが一致しません。'
      + '\ncommitするか、publish前に退避するか、ignoreしてください:\n'
      + `${dirty.stdout}`,
  );
