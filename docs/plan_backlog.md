...
## 請求項8を選ぶ条件（2026-07-28に裁定・実daemonで確認）

**難しさは推測せず、試して測る。** 振り分けを3段に割る。

| 段 | 誰が | 何を |
|---|---|---|
| 1 | 装置 | 係争資源が**切れる種類か**。path／symbolは面を分けられる。共有state／外部effectは分けても同じ資源に触り続ける |
| 2 | AI | 試すかどうか。隔離worktreeでの変換・focused test・再indexは費用がかかる |
| 3 | 装置 | 試した結果を五条件（ADR 0138）で測る。満たさなければ請求項7へ落とす |

段3は`resolveRuntimeSeam`が既に持っている。今回足したのは段1である。

`routeConflictTreatment`が`intentional_serial`を返すのは「変換が不可能」ではなく**事前宣言が
無いだけ**だった。実行時に初めて見つかった競合には事前宣言が無いので、既定だけを見ると
請求項8への道が無いように見える。`severability`と`transform_attemptable`を併せて返し、
`run status`の`treatment_advice`として運転側へ出す。**拒否ではなく助言**である（ADR 0128と
同じ規律）。

段2を装置が決めない理由は、費用のかかる判断だからである。毎回変換を試すかどうかは、操作する
AIが持つ文脈——残り時間、そのfileの重要度、他に走っている作業——で決まる。装置はそれを持たない。
...
こと**が要件である——発見時接続。これは製品の二段構え（計画時に完全な分断は得られない・
埋め合わせは実行段階の検知が持つ）の依存線への適用であり、静的発見の完全性を追う方向
（解決不能）へ設計を向けないこと。繋いだ線はready/frontier判定・gantt表示・席数標準の入力へ
即座に反映される。

## 入れ子表示v3: 既定で展開・その場表示・箱が育つ（オーナー裁定 2026-08-09）

v1/v2はトグル開閉型（親は1個の箱・開くと内側パネル）で受入済みだが、オーナーの本来の期待は
**標準状態で展開済み**——工程の箱がある「その場」に内部DAGが最初から描かれ、**親箱が中身に応じて
縦横に育つ**形。開閉は既定でなく例外操作（畳む側）へ逆転する。layoutエンジンへの影響が本体:
親箱の寸法が内部DAGで決まり、周囲の箱・辺・行がそれに押されて再配置される。再帰（箱の中の箱）でも
成立すること。v2の受入は書かれた仕様に対して正当であり、reopenしない——本項は仕様の写し漏れを
継承する続編である。
