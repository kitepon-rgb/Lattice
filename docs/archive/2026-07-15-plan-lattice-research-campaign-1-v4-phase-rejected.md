...
- Phase gate outcome: `rejected_for_behavior_evidence_identifiability`（mechanism evidenceは保持）
- predecessor: `lattice-research-campaign-1-v3`（[Phase-rejected archive](2026-07-15-plan-lattice-research-campaign-1-v3-phase-rejected.md)、SHA-256 `13a5ffa8580ac54dd1b63a556736b7fa888010483780660029cb938399a9baf7`）
- predecessor Phase gate: [ADR 0016](../adr/0016-rc1-v3-phase-gate-rejection.md)
...
- v4 Phase gate rejection: [ADR 0022](../adr/0022-rc1-v4-phase-gate-rejection.md)
...
- [x] TODO単位のfocused／related gateと軽量監査を各完了候補で一回行う。
- [x] P1 correctionだけを対象に独立refuterへ一回再監査し、同じfindingの無限シーソーをしない。
- [ ] source収束後のfull `npm run ci`を一回だけ実行する（P1でsource未収束へ戻ったためv5 Phase gateへ延期）。
- [x] v4をsupport／refuteする不変Decisionとevidenceを残す。
- [ ] RC1完了時だけplanをarchiveし、次campaignを正本化する（RC1未完了。ADR 0022のversion barrierでv5へ置換）。

Phase gateは[ADR 0022](../adr/0022-rc1-v4-phase-gate-rejection.md)と
[evidence](../evidence/2026-07-15-rc1-v4-phase-gate.md)でmachine supportをrejectした。実装はpre／post oracleを実行したが、
保存artifactがfull receipt、role、base SHA、entrypoint content、snapshot identityを持たず、postをpre receiptへ差し替えた世界を
区別できなかった。v4 artifactとmechanism evidenceは保持し、behavior evidenceをsnapshot-boundにするv5へ再compileする。
