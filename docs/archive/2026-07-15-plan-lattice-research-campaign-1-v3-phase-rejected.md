...
- Phase gate outcome: `rejected_for_causal_acceptance`（mechanism evidenceは保持）
- predecessor: `lattice-research-campaign-1-v2`（[archive](2026-07-15-plan-lattice-research-campaign-1-v2.md)、SHA-256 `170af6e1bc9b62b2b366b9c09a12861f3168808857aa96dc71c5cb51f03ad1ce`）
- bootstrap predecessor: `lattice-bootstrap-v1`（[archive](2026-07-15-plan-lattice-bootstrap-v1.md)、SHA-256 `78c07232cd2ede13dbd88bce02aa03b3591acc9ca1e39c21f7861398fc203a3b`）
...
- Phase gate rejection: [ADR 0016](../adr/0016-rc1-v3-phase-gate-rejection.md)
...
Phase gate Decision: [ADR 0016](../adr/0016-rc1-v3-phase-gate-rejection.md)。full regressionはgreenだったが、
condition別compiler、shared test-write脱落、fresh outcome preimage欠落によりcausal acceptanceをrejectした。
production transformとdigest chainはv4のhistorical mechanism evidenceとして保持する。


- [x] TODO単位の軽量監査を各完了候補で一回だけ行い、diff、受入条件、related test、手補正の有無を確認する。
- [x] 全TODO収束後にfull `npm run ci`を一回実行する。
- [x] Find→Dedup→独立反証→Critic→親裁定のPhase監査を一回行い、件数遷移と棄却理由を残す。
- [x] 成功／反証結果を`docs/evidence/`へ、再利用可能な実測・外部仕様を`rag/`へ還流する。
- [x] 完了したplanを`docs/archive/`へ退避し、次plan versionを正本化する。

Phase監査は16 raw findingを11 familyへdedupし、supported 5／refuted 6と裁定した。詳細は
[RC1 v3 Phase gate evidence](../evidence/2026-07-15-rc1-v3-phase-gate.md)を正本とする。
...
- dotagents repoはread-only参照のみ。orchestration Controlのproduct-owned stateはLatticeの`.git/`配下、
