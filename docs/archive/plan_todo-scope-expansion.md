...
## 入れ子表示の工程（plan_key `scope-split-viz-20260809`・オーナー裁定で本波へ昇格）

当初「別対応でよい」だった入れ子表示は、オーナー裁定（2026-08-09）で**今回の波に入れる**ことになった。
既にtaskが登録済みのscope-split planへは足さず（planへの後付けは本campaignの主題が戒める形）、
隣接planとして起こして同じ卓・同じ波で実行する。

#### v1 入れ子gantt描画（Lattice）
`parent_task_id`の系譜で、親taskの箱の内側に子task群の依存DAGを工程図として描く。行の折り畳みでは
ない——外から見れば親は1個の箱のまま、開くと箱の内側に内部工程図（graphvizのcluster相当・再帰可）。
対象はtodo-gantt-html／layout／svg／presentation。親を持たないtaskだけのplanでは現行描画と完全一致
（後方互換の負のコントロール）。データはまず合成fixture（parent_task_id入りのplan）で作る。

#### v2 入れ子表示の受入（親立会い）
受入条件（実測）: ①s3のsplitが作った実データ（parent_task_id系譜）で入れ子が描ける ②再帰（箱の中の箱）
が1段以上動く ③親無しplanの描画が無変更 ④公開工程表（gantt serve）で見える。

## 欲張り（当初案の記録・オーナー発案 2026-08-09）
