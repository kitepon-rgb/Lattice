...
（工程・ToDo の正本は Lattice store の `bridge-hub` planであり、active versionはstoreが選ぶ。初期ToDoの原本は[archive/plan_bridge-hub_tasks.md](plan_bridge-hub_tasks.md)へ移設済み。本書は目的・思想・判断理由・非目標・受入条件だけを所有する）
