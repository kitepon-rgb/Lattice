...
1. **共有surfaceの粒度。** 発火条件: 共有面が2つ以上の独立な依存塊を持つ実例が1件出たら着手する（ADR 0142）。 所有surfaceが依存する未宣言symbolを1つの共有fileへまとめるか、
   依存の塊ごとに分けるか。実データ1件では判断材料が足りない。
2. **外部挙動同等性の基準。** 裁定済み（ADR 0138 五条件——原pathの公開export面の保存）。 `rt-004`で裁定する。focused testの通過だけで足りるとは決めていない。
3. **所有surfaceのpath命名。** 裁定済み（ADR 0142 B）——名前は操作するAIが与える。機械の`<stem>.seam-<hash><ext>`は既定に留める。 現在は`<stem>.seam-<hash><ext>`である。人が読む名前へ変えるかは、
