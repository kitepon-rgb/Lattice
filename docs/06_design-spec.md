...
7. 右ペインはToDo storeを表示する（§6）。元plan Markdown本文は再表示しない。各ToDoの詳細は
   元文書への行対応を示し、アンカー検証が成立しない場合はfail closedでその旨を明示する。
8. レイアウトは縦流しとし、依存進行を上から下、レーンを横並びに配置する。
9. 図は空間を捨てない。配線帯の高さはその帯を通るedge数で決め、図全体の最大値を全段へ適用しない。
   依存edgeを持たないToDoは段の中で折り返して格子に並べ、横一列に伸ばさない。そのブロックは
   接続済みToDoの**上**へ置き、接続済みToDoが段の最下行になる（ADR 0068）。接続済みカードから配線帯へ
   降りる取り付け線が、真下の独立カードを貫いて別の依存に見えるのを構造的に消すため。独立ToDoは
   入辺を持たず必ず第0段に入り、第0段へは何も到着しないので、この配置で貫通経路が存在しなくなる。
10. 段を跳ぶ依存線は、両端カードの間にある**列境界の縦チャネル**を通る（ADR 0068）。図の右端の外へ出る
   回廊を持たない。候補は両端の列の間の境界（同一列なら左右の隣接境界）で、最も空いているものを選ぶ。
   チャネル幅は通行本数に応じて広げ、通行の無い境界は24pxのままとする。
11. 同じ列の真下へ繋ぐ線は、出発と到着で取り付け枠を1つ共有し、折れずに一直線で降ろす（ADR 0068）。
...
- 矢印頭は小さく（6px級）。線がノードを貫通しない（交差低減はレイアウトの仕事）。この不貫通は
  `test/todo-gantt-layout.test.mjs`の`assertNoRouteEntersBox`が全経路×全カードで検査し、除外を持たない。
  緩めるとADR 0068の誤読事故へ戻るので、除外を足さない。
...
- **作業記録**: 選択工程は通常詳細読取と同じbounded `note_context`を表示する。本文に加えて旧版由来、
  ...
  記録時刻は`<time datetime>`へUTCを保持し、閲覧者の現地時刻への変換はブラウザ側が行う。生成側の
  TZを焼き込まない——HTMLを作る側は閲覧者の地域を知らない。script不動作時はUTCがそのまま読める。
