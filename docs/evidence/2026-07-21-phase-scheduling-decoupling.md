...
Gantt/status関連32 testも32/32 green。その後の最終`npm test` full suite、`npm run check`、
`npm pack --dry-run --ignore-scripts`、Lattice store verifyはいずれもgreenで、packageへ
`lattice.plan_create_input.v3.schema.json`が収録されることを確認した。
