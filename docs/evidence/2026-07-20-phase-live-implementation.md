...
- static GanttはHTMLとdigest付きsidecarを発行し、`todo gantt status`が`current / stale / missing`を
  現在の決定的renderから判定する。片側欠落・改ざんは`GANTT_ARTIFACT_INVALID`として拒否する。
...
- `npm run check`: green
- `npm run ci`: green（Lattice 535/535、bundled sensor 2414 pass / 37 skip）
...
- Gantt status focused: missing、current、narrative driftによるstale、HTML digest改ざんのtyped拒否がgreen。
...
- **解消:** static Gantt artifactのcurrent/stale/missingを独立判定する`todo gantt status`を追加した。
...
敵対監査で残したHigh/hardening 2件は解消した。
