<!-- 出典: https://arxiv.org/pdf/2601.13295 | 取得日: 2026-07-15 | 取得方法: MarkItDown（PDF直取得、bytes確認） | 確度: 中（2026年preprint、600超の協調coding benchmark） -->
...
CooperBenchExpert-written features with potential conflicts but compatible solutions.Individual execution environmentsAgent 1’s GoalEnsure images mutable after saving to the disk.Virtual Machine 111:45am12:01pm12:03pm12:05pm12:10pm12:11pm...Agent 2’s GoalAuto backup when overwriting existing files.Virtual Machine 211:45am12:01pm12:03pm12:08pm12:15pm12:27pm...Chat between agentsAgent 1 12:01pmI will make Image.save function call _ensure_mutable. Agent 2 12:03pmThanks! I’ll anchor mine around the fp path handling block. We won’t conflict.Agent 2 12:27pmUpdate: I have finished my tasks.Agent 1 12:35pmWait, I now need your help to make sure to not return until the end of the function.Agent 2 12:27pmGot it. I have updated my patch.EvaluationMerge PatchesAgents patches should be compatible.Unit TestsAgent 1’s Tests+Agent 2’s Tests
...
et al., 2010).
In human societies, such cooperation relies on social intelligence: the ability to
...
Flexible `dspy.ToolCalls` parsing for varied formatstest_toolcalls_vague_match
test_tool_convert...no_input_params
test_tool_convert...args_lang_chainadapters/types/tool.py
tests/adapters/test_tool.pyFeature 1Minimal Python-call syntax parser for `ToolCalls`test_toolcalls_validate...string
test_toolcalls...multiple_calls
test_parse_python_calls_basic
test_parse_python_calls_multipleadapters/types/tool.py
tests/adapters/test_tool.pyFeature 2Minimal type coercion & unit parsing for `Tool` class arguments (safe, pre-validation)...Feature 3Feature 4-6 OmittedFeature PoolUsing repo state stanfordnlp/dspy #80412c as an exampleCompatiblePotentially ConflictingAll of the features can be implemented in a compatible way  Features are related to overlapping files. There is a potential for conflicts if not coordinated well.CooperBench : Why Coding Agents Cannot be Your Teammates Yet
...
Open-source  repos of different language with over 1K stars and dataset creator expertise Well documented Issue/PRHas new/updated tests<200 lines and 2 filesSet of 34 PRs for 12 different repos in 4 languagesStage I - Repository & PR selectionStage II - Feature creationStage III - Environment SetupReproducible ExecutionSanitize & formatManually creating test filesFeature PoolCurator + LLM IdeationCooperBench : Why Coding Agents Cannot be Your Teammates Yet
...
∑m∈M Solom(t), where Solom(t) denotes model m’s Solo success on t.
...
gpt-5claudeminimaxqwen coderqwen0.00.20.40.60.81.0Success rate0.480.280.470.260.360.140.220.130.060.050.00.20.40.60.81.0Relative Difficulty0.00.20.40.60.81.0SoloCoopCooperBench : Why Coding Agents Cannot be Your Teammates Yet
...
01020304050Percent of tasksclaudegpt-5minimaxqwenqwen coderWith commNo comm(a) Success rate020406080100Percent of tasksWith commNo comm(b) Conflict rate05101520Comm events (% of all events)20.0%16.3%13.6%6.2%3.3%PlanQuestionAnswerUpdateAck(c) Communication overheadCooperBench : Why Coding Agents Cannot be Your Teammates Yet
...
051015Avg turnsqwen coderqwenminimaxgpt-5claude1.13.210.815.514.5(a) Conversation turns010203040Percent of conversations0.0%6.8%17.8%5.1%37.1%Repeats same informationNear-duplicate status blocks(b) Repetition010203040Percent of conversations0.0%1.9%21.3%5.8%9.9%No reply to direct questionReply but ignores questionVague / non-answer(c) Unresponsiveness010203040Percent of conversations0.0%0.0%5.4%2.1%6.9%Plan drift / unilateral deviationHallucination (uncorrected)Hallucination (corrected)(d) HallucinationCooperBench : Why Coding Agents Cannot be Your Teammates Yet
...
I will modify outlines/prompts.py. I'm removing _template_from_str() and _template_from_file() methods. My create_jinja_env() function will call your get_global_filters().............Add global filter registryAdd global filter registryI will insert a block defining GLOBAL_FILTERS + register/unregister APIs.BashViewEditCommCooperBench : Why Coding Agents Cannot be Your Teammates Yet
...
I will add bypass check at lines 100-104 happens FIRST in get().............BashViewEditCommbypass code missingImplementation complete! .... ✓ Added bypass() context manager method ........ Which approach would you prefer? I want to ensure we don’t lose any functionality while resolving this conflict.............BashViewEditCommNo response at ALL.CooperBench : Why Coding Agents Cannot be Your Teammates Yet
...
Runs a quick check up front, then delegates the concrete insertion tasks with a clear plan.Insert octal_str in outlines/types/__init__.pyOn my branch I’ll insert: header → binary_str → octal_str.Inspects outlines/types/__init__.py to confirm the exact alphabetical insertion point.Locates the relevant files  to understand the surface area for multi-file editing.reads `src/click/termui.py` to understand how `click.edit` currently routes into the editor implementation.You implement the environment-isolation feature and I’ll implement multi-file editing support. Please avoid the backend implementation; I will handle it end-to-end.I won’t touch the backend editor implementation. I’ll implement environment isolation in the CLI-layer logic only.I’ll add the section header + octal_str in outlines/types/__init__.py; you add binary_str between them to keep alphabetical order.CooperBench : Why Coding Agents Cannot be Your Teammates Yet
...
I will modify llama-index-core/llama_index/core/base/llms/types.py lines 68–84 (ImageBlock.image_to_base64)............Edit types.py (ImageBlock.image_to_base64)I will NOT edit lines 68–84. My plan is to insert get_image_mimetype() AFTER line 84 (starting at new line 85).............Edit Prompt/conditional-filter logic in outlines/prompts.py (outside the inserted registry block).I will INSERT (no existing lines edited) into outlines/prompts.py between current line 14 and @dataclass Prompt at line 15. The inserted block defines GLOBAL_FILTERS + register/unregister APIs. Please avoid the inserted block.I already modified outlines/prompts.py line 7 (import field). I will edit only regions inside @dataclass PromptI checked the file… here are two clean options:
...
Which option do you prefer?............Add import reEdits `dirty_equals/__init__.py` to export both new classes.I’m seeing a potential overlap between our plans… I’ll check the current file state first, then we’ll coordinate a clean split.Let’s do option (1)… I’ve already added `import re` now. You add IsHash, then I’ll add IsRegex.CooperBench : Why Coding Agents Cannot be Your Teammates Yet
...
Katia Sycara. Theory of mind for multi-agent collaboration via large language models.
In
...
Avg. Lines Avg. Functions Avg. Files Easy Medium Hard
1774%
00%
320%
1763%
...
absolute gap ∆
AUC = AUCSolo − AUCCoop measures coordination cost but depends on baseline.
...
0.10.20.30.40.50.60.70.80.9Relative Difficulty0.00.20.40.60.81.0Task success rategpt-50.10.20.30.40.50.60.70.80.9Relative Difficultyclaude0.10.20.30.40.50.60.70.80.9Relative Difficultyminimax0.10.20.30.40.50.60.70.80.9Relative Difficultyqwen coder0.10.20.30.40.50.60.70.80.9Relative DifficultyqwenCoopSoloCooperBench : Why Coding Agents Cannot be Your Teammates Yet
...
0 coordination messages were sent (total_messages=0), so neither agent disclosed
