<!-- 出典: https://arxiv.org/pdf/2309.12499 | 取得日: 2026-07-15 | 取得方法: MarkItDown（PDF直取得、bytes確認） | 確度: 中〜高（Microsoft Research著者公開preprint） -->
...
MMS Recompute the edges incident on the
...
AM Add new node and edges by analyzing
...
ACC Add new node and edges by analyzing
...
DM Remove the node for M and edges in-
...
DCC Remove the edges incident on the class
...
Remove the node of the import state-

Rel(D, I, ImportedBy)

statement I

ment and edges incident on it.

Table 1. Rules for updating the dependency graph and for change may-impact analysis for atomic changes.

We refer to the dependency graphs before and after the updates by D and D′ respectively.
...
2.2.2 Change May-Impact Analysis. In the fifth step, CodePlan identifies the code blocks that may
...
2.3.2 Plan Execution. CodePlan iteratively selects a pending node in the plan graph and invokes
...
5.1 RQ1: How well CodePlan is able to localize and make the required changes to
...
5.2 RQ2: How important are the temporal and spatial contexts for CodePlan’s
...
5.3 RQ3: What are the key differentiators that allow CodePlan to outperform baselines
...
(MAPS 2022). Association for Computing Machinery, New York, NY, USA, 1–10. https://doi.org/10.1145/3520312.
