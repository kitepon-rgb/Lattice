 ...
 * the full real path: real LatticeSensor index + real ToolHandler.execute().
 *
...
describe('MCP staleness banner', () => {
  ...
  beforeEach(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-stale-banner-'));
    ...
    // files shared imports/calls, lattice_sensor_search responses would surface
  ...
  it('prepends a stale banner when the response references a pending file', async () => {
    ...
    const res = await handler.execute('lattice_sensor_search', { query: 'alphaOnly' });
  ...
  it('lists pending files under "Pending sync" in lattice_sensor_status', async () => {
    cg.watch({ debounceMs: 4000, inertForTests: true });
    await cg.waitUntilWatcherReady();
  ...
  it('surfaces the degraded state as its own section in lattice_sensor_status (#876)', async () => {
    degradeWatcher();
