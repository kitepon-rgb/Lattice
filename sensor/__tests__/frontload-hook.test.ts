 ...
 *
 * The Claude `UserPromptSubmit` front-load hook must inject LatticeSensor context
...
/** Make `dir` look indexed (isInitialized needs `.lattice/sensor/sensor.db`). */
function mkIndexed(dir: string): string {
  fs.mkdirSync(path.join(dir, '.lattice/sensor'), { recursive: true });
  fs.writeFileSync(path.join(dir, '.lattice/sensor', 'sensor.db'), '');
  return dir;
}
