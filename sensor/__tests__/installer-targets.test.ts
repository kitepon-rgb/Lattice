...
// A marker-delimited LatticeSensor block exactly as a previous installer
...
const LEGACY_BLOCK = [
  '<!-- LATTICE_SENSOR_START -->',
  '## LatticeSensor',
  '',
  'Prefer `lattice_sensor_search` / `lattice_sensor_callers` over grep.',
  '<!-- LATTICE_SENSOR_END -->',
].join('\n');
        ...
        describe(`location=${location}`, () => {
          ...
          it('install preserves a pre-existing sibling MCP server (where applicable)', () => {
            ...
            if (target.id === 'opencode') {
              ...
              expect(after.mcp.lattice/sensor).toBeDefined();
            } else {
              expect(after.mcpServers.other).toBeDefined();
              expect(after.mcpServers.lattice/sensor).toBeDefined();
            }
          });
...
describe('Installer targets — partial-state idempotency', () => {
  ...
  it('codex: install writes config.toml AND the AGENTS.md latticeSensor block (#704)', () => {
    ...
    expect(body).toContain('## LatticeSensor');
    expect(body).toContain('latticeSensor explore');
  ...
  it('codex: install replaces a legacy AGENTS.md latticeSensor block with the current one, keeping user content', () => {
    const codex = getTarget('codex')!;
    ...
    expect(body).not.toContain('Prefer `lattice_sensor_search`');
    expect(body).toContain('latticeSensor explore');
  ...
  it('opencode: prefers .jsonc when both .json and .jsonc exist', () => {
    ...
    expect(jsonText).not.toContain('latticeSensor');
  });
  ...
  it('opencode: preserves line and block comments through install + idempotent re-run', () => {
    ...
    expect(afterInstall).toContain('"latticeSensor"');
  ...
  it('opencode: install writes the AGENTS.md latticeSensor block (#704)', () => {
    const opencode = getTarget('opencode')!;
    const result = opencode.install('global', { autoAllow: true });
    const agentsMd = path.join(tmpHome, '.config', 'opencode', 'AGENTS.md');
    expect(fs.existsSync(agentsMd)).toBe(true);
    expect(fs.readFileSync(agentsMd, 'utf-8')).toContain('latticeSensor explore');
    expect(result.files.find((f) => f.path.endsWith('AGENTS.md'))?.action).toBe('created');
  });

  it('opencode: install replaces a legacy AGENTS.md latticeSensor block, preserving user content', () => {
    const opencode = getTarget('opencode')!;
    const dir = path.join(tmpHome, '.config', 'opencode');
    fs.mkdirSync(dir, { recursive: true });
    const agentsMd = path.join(dir, 'AGENTS.md');
  ...
  it('opencode: uninstall strips a leftover latticeSensor block from AGENTS.md, keeping user content', () => {
    const opencode = getTarget('opencode')!;
    const dir = path.join(tmpHome, '.config', 'opencode');
    fs.mkdirSync(dir, { recursive: true });
    const agentsMd = path.join(dir, 'AGENTS.md');
    ...
    expect(body).toContain('# My personal opencode instructions');
    expect(body).toContain('Always respond in pirate.');
    expect(body).not.toContain('LATTICE_SENSOR_START');
  });
  ...
  it('gemini: install writes settings.json (mcpServers.lattice/sensor) and the GEMINI.md block (#704)', () => {
    ...
    expect(fs.readFileSync(geminiMd, 'utf-8')).toContain('latticeSensor explore');

    const cfg = JSON.parse(fs.readFileSync(settings, 'utf-8'));
    expect(cfg.mcpServers.lattice/sensor).toEqual({ type: 'stdio', command: 'latticeSensor', args: ['serve', '--mcp'] });
  });
  ...
  it('gemini: uninstall strips latticeSensor but leaves pre-existing settings (security.auth) intact', () => {
    const gemini = getTarget('gemini')!;
    const settings = path.join(tmpHome, '.gemini', 'settings.json');
    fs.mkdirSync(path.dirname(settings), { recursive: true });
    fs.writeFileSync(settings, JSON.stringify({
  ...
  it('gemini: uninstall strips a leftover GEMINI.md latticeSensor block, keeping user content', () => {
    const gemini = getTarget('gemini')!;
    const geminiMd = path.join(tmpHome, '.gemini', 'GEMINI.md');
  ...
  it('kiro: install writes settings/mcp.json (mcpServers.lattice/sensor) and no steering doc (#529)', () => {
    ...
    const steering = path.join(tmpHome, '.kiro', 'steering', 'latticeSensor.md');
  ...
  it('kiro: install deletes a leftover steering latticeSensor.md (self-heal) (#529)', () => {
    const kiro = getTarget('kiro')!;
    const steering = path.join(tmpHome, '.kiro', 'steering', 'latticeSensor.md');
  ...
  it('kiro: uninstall strips latticeSensor but leaves sibling MCP servers intact', () => {
    const kiro = getTarget('kiro')!;
    const mcp = path.join(tmpHome, '.kiro', 'settings', 'mcp.json');
    fs.mkdirSync(path.dirname(mcp), { recursive: true });
    fs.writeFileSync(mcp, JSON.stringify({
    ...
    const after = JSON.parse(fs.readFileSync(mcp, 'utf-8'));
    expect(after.mcpServers.other).toBeDefined();
    expect(after.mcpServers.lattice/sensor).toBeUndefined();
  });

  it('kiro: uninstall removes a leftover steering latticeSensor.md file outright', () => {
    const kiro = getTarget('kiro')!;
    const steering = path.join(tmpHome, '.kiro', 'steering', 'latticeSensor.md');
    fs.mkdirSync(path.dirname(steering), { recursive: true });
    fs.writeFileSync(steering, `${LEGACY_BLOCK}\n`);
  ...
  it('kiro: uninstall removes our steering doc but leaves a sibling (product.md) untouched', () => {
    ...
    const ours = path.join(tmpHome, '.kiro', 'steering', 'latticeSensor.md');
  ...
  it('kiro: local install writes ./.kiro/settings/mcp.json and no steering doc (#529)', () => {
    ...
    expect(paths.some((p) => p.endsWith('/.kiro/steering/latticeSensor.md'))).toBe(false);
  });
  ...
  it('antigravity: install writes to LEGACY ~/.gemini/antigravity/mcp_config.json when no migration marker', () => {
    ...
    expect(cfg.mcpServers.lattice/sensor).toBeDefined();
  ...
  it('antigravity: entry has NO `type` field (Antigravity rejects entries with it)', () => {
    ...
    expect(cfg.mcpServers.lattice/sensor.type).toBeUndefined();
    expect(cfg.mcpServers.lattice/sensor.command).toBeDefined();
    expect(cfg.mcpServers.lattice/sensor.args).toEqual(['serve', '--mcp']);
  });

  it('antigravity: install migrates a legacy latticeSensor entry to the unified path when marker appears', () => {
    const antigravity = getTarget('antigravity')!;
    // Simulate: user installed on the legacy path, then Antigravity
    // migrated their config (dropped the `.migrated` marker + created
    // the unified file). Re-running latticeSensor install should land
    // latticeSensor in the new file AND strip the stale legacy entry.
    const legacyFile = path.join(tmpHome, '.gemini', 'antigravity', 'mcp_config.json');
    fs.mkdirSync(path.dirname(legacyFile), { recursive: true });
    fs.writeFileSync(legacyFile, JSON.stringify({
      mcpServers: { latticeSensor: { command: 'latticeSensor', args: ['serve', '--mcp'] } },
    }, null, 2) + '\n');
    fs.mkdirSync(path.join(tmpHome, '.gemini', 'config'), { recursive: true });
    fs.writeFileSync(path.join(tmpHome, '.gemini', 'config', '.migrated'), '');
    ...
    expect(unified.mcpServers.lattice/sensor).toBeDefined();
    // Legacy file's latticeSensor entry got stripped.
  ...
  it('antigravity: uninstall removes only latticeSensor, sibling MCP server survives', () => {
    const antigravity = getTarget('antigravity')!;
    const mcpFile = path.join(tmpHome, '.gemini', 'antigravity', 'mcp_config.json');
    fs.mkdirSync(path.dirname(mcpFile), { recursive: true });
    fs.writeFileSync(mcpFile, JSON.stringify({
  ...
  it('antigravity: uninstall sweeps BOTH legacy and unified paths (handles migration half-state)', () => {
    const antigravity = getTarget('antigravity')!;
    // User had latticeSensor in BOTH files (e.g. legacy install + post-migration
  ...
  it('gemini + antigravity: both installed coexist (separate MCP files, shared GEMINI.md)', () => {
    ...
    expect(cliCfg.mcpServers.lattice/sensor).toBeDefined();
    expect(ideCfg.mcpServers.lattice/sensor).toBeDefined();
    ...
    expect(cliAfter.mcpServers.lattice/sensor).toBeDefined();
  });

  it('hermes: install adds latticeSensor MCP server and cli toolset, preserving existing yaml', () => {
    ...
    expect(body).toContain('  latticeSensor:\n    command: latticeSensor');
    expect(body).toContain('    - hermes-cli');
    expect(body).toContain('    - mcp-latticeSensor');
  ...
  it('hermes: uninstall removes only latticeSensor MCP server and toolset entry', () => {
    const hermes = getTarget('hermes')!;
    const config = path.join(tmpHome, '.hermes', 'config.yaml');
    fs.mkdirSync(path.dirname(config), { recursive: true });
    ...
    const body = fs.readFileSync(config, 'utf-8');
    expect(body).not.toContain('latticeSensor:');
    expect(body).not.toContain('mcp-latticeSensor');
  ...
  // the next sibling key, truncated the cli block, and spliced `- mcp-latticeSensor`
  ...
  it('hermes: install preserves PyYAML-default list-at-same-indent style (issue #456)', () => {
    ...
    // mcp-latticeSensor appended at the same 2-space indent as existing items
    ...
    // start with `cli:` and not contain a stray 4-space `mcp-latticeSensor`
  ...
  it('hermes: uninstall reverses the install on a PyYAML-default config', () => {
    ...
    expect(installed).toContain('- mcp-latticeSensor');
    expect(installed).toContain('latticeSensor:');

    hermes.uninstall('global');
    const body = fs.readFileSync(config, 'utf-8');
    expect(body).not.toContain('mcp-latticeSensor');
    expect(body).not.toContain('command: latticeSensor');
    expect(body).toContain('  cli:\n  - hermes-cli\n  - browser');
    expect(body).toContain('  telegram:\n  - hermes-telegram');
  });

  it('opencode: uninstall removes only mcp.lattice/sensor, preserves comments and siblings', () => {
    const opencode = getTarget('opencode')!;
    const dir = path.join(tmpHome, '.config', 'opencode');
    fs.mkdirSync(dir, { recursive: true });
    const file = path.join(dir, 'opencode.jsonc');
    ...
    expect(afterUninstall).not.toContain('latticeSensor');
    expect(afterUninstall).toContain('// important comment');
    expect(afterUninstall).toContain('"other"');
  });

  it('codex: user-added key inside [mcp_servers.lattice/sensor] survives idempotent re-install', () => {
    const codex = getTarget('codex')!;
    ...
    // latticeSensor block exclusively.
    const second = codex.install('global', { autoAllow: false });
  ...
  it('claude: install creates the CLAUDE.md latticeSensor block (#704)', () => {
    const claude = getTarget('claude')!;
    const result = claude.install('local', { autoAllow: false });
  ...
  it('claude: install replaces a legacy CLAUDE.md latticeSensor block, keeping user content', () => {
    const claude = getTarget('claude')!;
    const claudeMd = path.join(tmpCwd, '.claude', 'CLAUDE.md');
  ...
  it('claude: local install migrates a legacy ./.claude.json latticeSensor entry into ./.mcp.json', () => {
    const claude = getTarget('claude')!;
    const legacy = path.join(tmpCwd, '.claude.json');
    fs.writeFileSync(
      legacy,
      JSON.stringify({ mcpServers: { latticeSensor: { type: 'stdio', command: 'latticeSensor', args: ['serve', '--mcp'] } } }, null, 2),
    );

    claude.install('local', { autoAllow: false });

    // latticeSensor now lives in .mcp.json; the legacy file (which held only
    // latticeSensor) is gone.
    const mcp = JSON.parse(fs.readFileSync(path.join(tmpCwd, '.mcp.json'), 'utf-8'));
    expect(mcp.mcpServers.lattice/sensor).toBeDefined();
  ...
  it('claude: legacy ./.claude.json migration preserves sibling servers and unrelated keys', () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        mcpServers: {
          latticeSensor: { type: 'stdio', command: 'latticeSensor', args: ['serve', '--mcp'] },
          other: { command: 'x' },
        },
        somethingElse: true,
      }, null, 2),
    ...
    // Only latticeSensor is stripped from the legacy file; siblings survive.
  ...
  it('claude: uninstall strips latticeSensor from ./.mcp.json and a legacy ./.claude.json', () => {
    const claude = getTarget('claude')!;
    // A user left with both the working .mcp.json and a stale .claude.json.
    fs.writeFileSync(
      path.join(tmpCwd, '.mcp.json'),
      JSON.stringify({ mcpServers: { latticeSensor: { command: 'latticeSensor' } } }, null, 2),
    );
    fs.writeFileSync(
      path.join(tmpCwd, '.claude.json'),
      JSON.stringify({ mcpServers: { latticeSensor: { command: 'latticeSensor' }, other: { command: 'x' } } }, null, 2),
    );
    ...
    expect(legacy.mcpServers.lattice/sensor).toBeUndefined();
    expect(legacy.mcpServers.other).toBeDefined();
  });

  // ---- Legacy auto-sync hook cleanup ----
  // Pre-0.8 installs wrote `latticeSensor mark-dirty` / `sync-if-dirty`
  ...
  function legacyHookSettings(): Record<string, any> {
    return {
      hooks: {
        PostToolUse: [
          { matcher: 'Edit|Write', hooks: [{ type: 'command', command: 'latticeSensor mark-dirty', async: true }] },
        ],
        Stop: [
          { hooks: [{ type: 'command', command: 'latticeSensor sync-if-dirty' }] },
  ...
  it('claude: install strips stale latticeSensor auto-sync hooks but keeps the user\'s GitKraken hook', () => {
    const claude = getTarget('claude')!;
    ...
    expect(stopCommands).not.toContain('latticeSensor sync-if-dirty');
    ...
    expect(after.permissions?.allow).toContain('mcp__lattice_sensor__*');
  });
    ...
    const file = seedSettings('global', {
      hooks: {
        Stop: [
          {
            hooks: [
              { type: 'command', command: 'latticeSensor sync-if-dirty' },
  ...
  it('claude: cleanupLegacyHooks is a byte-for-byte no-op without latticeSensor hooks', () => {
    const original =
  ...
  it('claude: uninstall strips stale hooks written in the npx form (local)', () => {
    ...
    const file = seedSettings('local', {
      hooks: {
        PostToolUse: [
          { matcher: 'Edit|Write', hooks: [{ type: 'command', command: 'npx @colbymchenry/latticeSensor mark-dirty', async: true }] },
        ],
        Stop: [
          { hooks: [{ type: 'command', command: 'npx @colbymchenry/latticeSensor sync-if-dirty' }] },
        ],
      },
    });
  ...
  // `latticeSensor prompt-hook`. Must write/remove surgically, be idempotent, and
  ...
  it('claude: install with promptHook:true writes the UserPromptSubmit hook (alongside permissions)', () => {
    ...
    expect(promptCommands(s)).toContain('latticeSensor prompt-hook');
    expect(s.permissions?.allow).toContain('mcp__lattice_sensor__*');
  });
  ...
  it('claude: install without promptHook does NOT add the hook', () => {
    ...
    claude.install('global', { autoAllow: true });
    const s = JSON.parse(fs.readFileSync(path.join(tmpHome, '.claude', 'settings.json'), 'utf-8'));
    expect(promptCommands(s)).not.toContain('latticeSensor prompt-hook');
  });
  ...
  it('claude: install with promptHook:true is idempotent (no duplicate, byte-identical re-run)', () => {
    ...
    expect(promptCommands(s).filter((c: string) => c === 'latticeSensor prompt-hook')).toHaveLength(1);
  });
  ...
  it('claude: writePromptHookEntry preserves a sibling UserPromptSubmit hook', () => {
    ...
    expect(promptCommands(s)).toEqual(['my-own-hook', 'latticeSensor prompt-hook']);
  });
  ...
  it('claude: uninstall removes the prompt hook but keeps the user\'s sibling', () => {
    const file = seedSettings('global', {
      hooks: {
        UserPromptSubmit: [
          { hooks: [{ type: 'command', command: 'latticeSensor prompt-hook' }] },
  ...
  it('claude: removePromptHookEntry leaves the legacy auto-sync hook untouched', () => {
    const file = seedSettings('global', {
      hooks: {
        UserPromptSubmit: [{ hooks: [{ type: 'command', command: 'latticeSensor prompt-hook' }] }],
        Stop: [{ hooks: [{ type: 'command', command: 'latticeSensor sync-if-dirty' }] }],
      },
    });
    ...
    expect(stopCmds).toContain('latticeSensor sync-if-dirty');
  });
});
...
describe('Installer targets — TOML serializer (Codex backbone)', () => {
  it('builds a [mcp_servers.lattice/sensor] block with command + args', () => {
    const block = buildTomlTable('mcp_servers.lattice/sensor', {
      command: 'latticeSensor',
      args: ['serve', '--mcp'],
    });
    expect(block).toContain('[mcp_servers.lattice/sensor]');
    expect(block).toContain('command = "latticeSensor"');
    expect(block).toContain('args = ["serve", "--mcp"]');
  });

  it('upsert inserts into empty content', () => {
    const block = buildTomlTable('mcp_servers.lattice/sensor', { command: 'latticeSensor', args: ['serve'] });
    const { content, action } = upsertTomlTable('', 'mcp_servers.lattice/sensor', block);
    expect(action).toBe('inserted');
    expect(content.startsWith('[mcp_servers.lattice/sensor]')).toBe(true);
  });

  it('upsert is idempotent — second call returns unchanged', () => {
    const block = buildTomlTable('mcp_servers.lattice/sensor', { command: 'latticeSensor', args: ['serve'] });
    const first = upsertTomlTable('', 'mcp_servers.lattice/sensor', block);
    const second = upsertTomlTable(first.content, 'mcp_servers.lattice/sensor', block);
  ...
  it('upsert replaces an existing block in place, preserving sibling tables', () => {
    const existing = [
      ...
      'foo = "bar"',
      '',
      '[mcp_servers.lattice/sensor]',
      'command = "old-latticeSensor"',
      'args = ["old"]',
      '',
      '[zzz]',
      'baz = "qux"',
    ...
    const newBlock = buildTomlTable('mcp_servers.lattice/sensor', {
      command: 'latticeSensor',
      args: ['serve', '--mcp'],
    });
    const { content, action } = upsertTomlTable(existing, 'mcp_servers.lattice/sensor', newBlock);
    expect(action).toBe('replaced');
    expect(content).toContain('[other_table]');
    expect(content).toContain('foo = "bar"');
    expect(content).toContain('[zzz]');
    expect(content).toContain('baz = "qux"');
    expect(content).toContain('command = "latticeSensor"');
    expect(content).not.toContain('old-latticeSensor');
  });
  ...
  it('removeTomlTable strips the block and preserves siblings', () => {
    ...
    const { content, action } = removeTomlTable(existing, 'mcp_servers.lattice/sensor');
    expect(action).toBe('removed');
    expect(content).toContain('[other_table]');
    expect(content).toContain('foo = "bar"');
    expect(content).not.toContain('mcp_servers.lattice/sensor');
  });
  ...
  it('upsert preserves an array-of-tables sibling [[foo]]', () => {
    ...
    const block = buildTomlTable('mcp_servers.lattice/sensor', { command: 'latticeSensor', args: ['serve'] });
    const { content } = upsertTomlTable(existing, 'mcp_servers.lattice/sensor', block);
    expect(content.match(/\[\[foo\]\]/g)?.length).toBe(2);
    expect(content).toContain('[mcp_servers.lattice/sensor]');
  });
});

describe('Installer — uninstallTargets sweep (latticeSensor uninstall)', () => {
  let tmpHome: string;
  let tmpCwd: string;
  let origCwd: string;
  let homeRestore: { restore: () => void };
...
describe('Installer — refreshTargets sweep (latticeSensor install --refresh)', () => {
  let tmpHome: string;
  let tmpCwd: string;
  let origCwd: string;
  let homeRestore: { restore: () => void };
  ...
  it('rewrites a stale instructions block a previous version left, and reports refreshed', () => {
    ...
    expect(md).not.toContain('lattice_sensor_search');
    expect(md).toContain('lattice_sensor_explore');
  });
...
describe('Installer — Cursor rules file cleanup on uninstall', () => {
  ...
  const rulesFile = () => path.join(process.cwd(), '.cursor', 'rules', 'latticeSensor.mdc');
  ...
  const MDC_FRONTMATTER = [
    '---',
    'description: LatticeSensor MCP usage guide — when to use which tool',
    'alwaysApply: true',
    '---',
    '',
  ].join('\n');
  ...
  it('uninstall deletes a leftover latticeSensor.mdc entirely (no orphaned frontmatter left behind)', () => {
  ...
  it('install self-heals a leftover latticeSensor.mdc (#529)', () => {
    plantLegacyRulesFile();
    const result = cursor.install('local', { autoAllow: true });
    expect(fs.existsSync(rulesFile())).toBe(false);
    expect(result.files.some((f) => f.path.endsWith('latticeSensor.mdc') && f.action === 'removed')).toBe(true);
  });

  it('uninstall preserves user content added outside the latticeSensor markers (strips only our block)', () => {
    ...
    expect(after).not.toContain('lattice_sensor_search');
    expect(after).not.toContain('LATTICE_SENSOR_START');
  });
});
...
describe('Installer targets — opencode XDG config path (#535)', () => {
  ...
  it('greenfield: targets ~/.config/opencode even when the dir does not exist yet (#535)', () => {
    ...
    // ~/.config/opencode itself on first run; installing latticeSensor FIRST
  ...
  it('install self-heals a pre-#535 %APPDATA% entry, preserving siblings and comments', () => {
    // A previous latticeSensor version wrote into %APPDATA%/opencode. The user
    ...
    fs.writeFileSync(path.join(legacyDir(), 'opencode.jsonc'), [
      ...
      '  "$schema": "https://opencode.ai/config.json",',
      '  "mcp": {',
      '    "latticeSensor": { "type": "local", "command": ["latticeSensor", "serve", "--mcp"], "enabled": true },',
    ...
    expect(legacyText).not.toContain('latticeSensor');
  ...
  it('uninstall sweeps the legacy %APPDATA% entry too (no prior re-install needed)', () => {
    // A user on the broken version goes straight to `latticeSensor uninstall`:
    ...
    expect(fs.readFileSync(path.join(legacyDir(), 'opencode.json'), 'utf-8')).not.toContain('latticeSensor');
