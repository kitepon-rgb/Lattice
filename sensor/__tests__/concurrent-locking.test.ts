...
describe('issue #238 — connection PRAGMAs (#1)', () => {
  ...
  beforeAll(() => {
    ...
    conn = DatabaseConnection.initialize(path.join(dir, 'sensor.db'));
  });
...
describe('issue #238 — WAL lets a reader proceed during a writer', () => {
  ...
  it('a read on a 2nd connection succeeds while a writer holds the lock', () => {
    const dbPath = path.join(dir, 'sensor.db');
...
describe('issue #238 — ToolHandler reuses the default instance (#2)', () => {
  ...
  it('getLatticeSensor(defaultRoot) returns the default instance, not a new connection', () => {
    const openSpy = vi.spyOn(LatticeSensor, 'openSync');
    try {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const resolved = (handler as any).getLatticeSensor(root);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const nested = (handler as any).getLatticeSensor(path.join(root, 'does', 'not', 'exist'));
  ...
  it('concurrent read tool calls (mixed projectPath) all succeed without "database is locked"', async () => {
    ...
    try {
      const calls: Promise<{ content: Array<{ text: string }>; isError?: boolean }>[] = [
        handler.execute('lattice_sensor_search', { query: 'helper' }),
        handler.execute('lattice_sensor_search', { query: 'helper', projectPath: root }),
        handler.execute('lattice_sensor_callers', { symbol: 'helper', projectPath: root }),
        handler.execute('lattice_sensor_callees', { symbol: 'main' }),
        handler.execute('lattice_sensor_files', { projectPath: root }),
        handler.execute('lattice_sensor_status', { projectPath: root }),
      ];
