 ...
 *
 * `latticeSensor index` runs in a child re-exec'd with `--liftoff-only` whose parent
...
describe.skipIf(process.platform === 'win32')('index/init orphan supervision (#999)', () => {
  ...
  it("self-terminates when its parent is SIGKILL'd mid-index", async () => {
    ...
    // LATTICE_SENSOR_NO_WATCHDOG=1 isolates the ppid (orphan) path from the liveness
    // child; LATTICE_SENSOR_PPID_POLL_MS=200 keeps it responsive in test.
    const childSrc = `
    ...
    const wrapperSrc = `
      ...
      const child = spawn(process.execPath, ['-e', ${JSON.stringify(childSrc)}], {
        ...
        env: { ...process.env, LATTICE_SENSOR_NO_WATCHDOG: '1', LATTICE_SENSOR_PPID_POLL_MS: '200', LATTICE_SENSOR_WASM_RELAUNCHED: '1' },
        detached: true,
      });
      child.unref();
