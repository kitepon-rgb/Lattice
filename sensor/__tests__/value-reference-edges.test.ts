 ...
 * "change this constant, affect its readers". Default on; LATTICE_SENSOR_VALUE_REFS=0
...
function valueRefReaders(cg: LatticeSensor, constName: string): string[] {
...
describe('value-reference edges', () => {
  ...
  beforeEach(() => {
    dir = fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-valueref-'));
  });
  afterEach(() => {
    cg?.destroy();
    cg = undefined;
  ...
  function index(): LatticeSensor {
    const g = LatticeSensor.initSync(dir, { config: { include: ['**/*.ts', '**/*.tsx'], exclude: [] } });
    return g;
  }
  ...
  it('emits nothing when LATTICE_SENSOR_VALUE_REFS=0', async () => {
    const prev = process.env.LATTICE_SENSOR_VALUE_REFS;
    ...
    try {
      ...
      if (prev === undefined) delete process.env.LATTICE_SENSOR_VALUE_REFS;
      else process.env.LATTICE_SENSOR_VALUE_REFS = prev;
    }
  });
});
