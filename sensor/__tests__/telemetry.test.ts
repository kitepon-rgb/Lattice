...
describe('Telemetry', () => {
  ...
  beforeEach(() => {
    dir = fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-telemetry-'));
    calls = [];
    stderrLines = [];
  ...
  describe('consent precedence', () => {
    ...
    it('DO_NOT_TRACK beats everything, including a forced-on env and config', () => {
      const t = make({ env: { DO_NOT_TRACK: '1', LATTICE_SENSOR_TELEMETRY: '1' } });
      t.setEnabled(true, 'cli');
      expect(t.getStatus()).toMatchObject({ enabled: false, decidedBy: 'DO_NOT_TRACK' });
    });

    it('LATTICE_SENSOR_TELEMETRY env beats the stored config in both directions', () => {
      const t = make({ env: { LATTICE_SENSOR_TELEMETRY: '0' } });
      t.setEnabled(true, 'cli');
      expect(t.getStatus()).toMatchObject({ enabled: false, decidedBy: 'LATTICE_SENSOR_TELEMETRY' });

      const t2 = make({ env: { LATTICE_SENSOR_TELEMETRY: '1' } });
      t2.setEnabled(false, 'cli');
      expect(t2.getStatus()).toMatchObject({ enabled: true, decidedBy: 'LATTICE_SENSOR_TELEMETRY' });
    });
  ...
  describe('off is off', () => {
    it('disabled: records nothing, sends nothing, creates no files', async () => {
      ...
      const t = make({ env: { LATTICE_SENSOR_TELEMETRY: '0' }, fetchImpl: fetchSpy });
      t.recordUsage('mcp_tool', 'lattice_sensor_explore', true);
  ...
  describe('first-run notice & machine id', () => {
    it('recording only buffers — no notice, no config until something is sent', async () => {
      const t = make();
      t.recordUsage('mcp_tool', 'lattice_sensor_explore', true);
      t.recordUsage('mcp_tool', 'lattice_sensor_node', true);
    ...
    it('prints the notice exactly once, before the first actual send', async () => {
      ...
      expect(stderrLines[0]).toContain('lattice sensor telemetry off');
      expect(stderrLines[0]).toContain('LATTICE_SENSOR_TELEMETRY=0');
  ...
  describe('rollups & sending', () => {
    it('aggregates per (day, kind, name, client) and sends only completed days', async () => {
      ...
      t.recordUsage('mcp_tool', 'lattice_sensor_explore', true, client);
      t.recordUsage('mcp_tool', 'lattice_sensor_explore', false, client);
      t.recordUsage('mcp_tool', 'lattice_sensor_explore', true, client);
      ...
      const explore = events.find((e) => e.props.name === 'lattice_sensor_explore')!;
    ...
    it('uses the production endpoint by default and honors the env override', async () => {
      ...
      const t2 = make({ env: { LATTICE_SENSOR_TELEMETRY_ENDPOINT: 'http://localhost:9999/v1/events' } });
  ...
  describe('protocol safety', () => {
    it('never writes to stdout', async () => {
      ...
      const t = make({ env: { LATTICE_SENSOR_TELEMETRY_DEBUG: '1' } });
      t.recordUsage('mcp_tool', 'lattice_sensor_explore', true);
      t.recordLifecycle('install', { scope: 'local', kind: 'fresh' });
      await t.flushNow();
