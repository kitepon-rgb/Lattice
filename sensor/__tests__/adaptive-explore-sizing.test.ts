/**
 * Regression test for adaptive `lattice_sensor_explore` sizing — sibling
 ...
 * >= MIN_SIBLINGS (3) implementers — `lattice_sensor_explore` renders it as a
 ...
 * pipeline steps stay full). Default ON; LATTICE_SENSOR_ADAPTIVE_EXPLORE=0 disables.
 *
...
describe('adaptive lattice_sensor_explore sizing — sibling skeletonization', () => {
  let testDir: string;
  ...
  beforeAll(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-adaptive-explore-'));
...
`
  ...
  beforeEach(() => {
    ...
    delete process.env.LATTICE_SENSOR_ADAPTIVE_EXPLORE;
  });
  ...
  it('skeletonizes off-spine polymorphic siblings (bodies elided, signatures kept)', async () => {
    const result = await handler.execute('lattice_sensor_explore', { query: QUERY, maxFiles: 12 });
  ...
  it('LATTICE_SENSOR_ADAPTIVE_EXPLORE=0 disables skeletonization (siblings render full)', async () => {
    process.env.LATTICE_SENSOR_ADAPTIVE_EXPLORE = '0';
    try {
      const result = await handler.execute('lattice_sensor_explore', { query: QUERY, maxFiles: 12 });
      const text = result.content?.[0]?.text ?? '';
  ...
  it('spares an off-spine sibling when the agent NAMED a callable in it (RealCall fix)', async () => {
    const result = await handler.execute('lattice_sensor_explore', { query: SPARE_QUERY, maxFiles: 15 });
    const text = result.content?.[0]?.text ?? '';
    expect(text).toContain('**Flow (call path among the symbols you queried)');
  ...
  it('naming a SHARED/polymorphic method does not spare the siblings (uniqueness-aware)', async () => {
    ...
    const result = await handler.execute('lattice_sensor_explore', { query: `${QUERY} intercept`, maxFiles: 12 });
    const text = result.content?.[0]?.text ?? '';
