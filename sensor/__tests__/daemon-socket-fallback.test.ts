 ...
 *
 *   1. Lock acquisition `link()`s a temp file onto `.lattice/sensor/daemon.pid` for
...
describe('getDaemonSocketCandidates (#997)', () => {
  it.runIf(POSIX)('returns [in-project, tmpdir] for a normal short path', () => {
    ...
    expect(candidates[0]).toBe(path.join(root, '.lattice/sensor', 'daemon.sock'));
  ...
  it.runIf(POSIX)('drops straight to [tmpdir] when the in-project path is too long', () => {
    // A deep root pushes `.lattice/sensor/daemon.sock` past the POSIX socket limit.
...
describe('bindFirstUsableSocket (#997)', () => {
  it('binds the first candidate when it works, without relocating', async () => {
    ...
    const result = await bindFirstUsableSocket(
      ['/proj/.lattice/sensor/daemon.sock', '/tmp/fallback.sock'],
      (p) => { tried.push(p); return Promise.resolve(fakeServer(p)); },
      { onRelocate: (from, to) => relocations.push(`${from}->${to}`) },
    );
    expect(result.socketPath).toBe('/proj/.lattice/sensor/daemon.sock');
    expect(tried).toEqual(['/proj/.lattice/sensor/daemon.sock']); // never touched the fallback
  ...
  it('relocates to the tmpdir fallback when the in-project bind throws ENOTSUP', async () => {
    ...
    const result = await bindFirstUsableSocket(
      ['/exfat/proj/.lattice/sensor/daemon.sock', '/tmp/fallback.sock'],
      (p) => {
        tried.push(p);
    ...
    expect(tried).toEqual(['/exfat/proj/.lattice/sensor/daemon.sock', '/tmp/fallback.sock']);
    expect(relocations).toEqual([
      ['/exfat/proj/.lattice/sensor/daemon.sock', '/tmp/fallback.sock', 'ENOTSUP'],
    ]);
  });
  ...
  it('does NOT relocate on EADDRINUSE — it propagates even with a fallback present', async () => {
    ...
    expect(tried).toEqual(['/proj/.lattice/sensor/daemon.sock']); // fallback never tried
  });
  ...
  it('relocates on an UNEXPECTED errno too — the policy is "anything but EADDRINUSE", not a fixed list', async () => {
    ...
    // such surprise.
    const result = await bindFirstUsableSocket(
      ['/odd/proj/.lattice/sensor/daemon.sock', '/tmp/fallback.sock'],
