/**
 * Read-only MCP ToolAnnotations on every latticeSensor tool (issue #1018).
 *
 * Every latticeSensor tool is query-only — it reads the pre-built index and never
 ...
 * latticeSensor tools were blocked there even though they only read.
 *
 ...
 * rewrites lattice_sensor_explore's description via spread), and the no-default-
...
describe('Read-only annotations on the latticeSensor MCP tools (#1018)', () => {
...
describe('Live tool surface keeps annotations with a project open (#1018)', () => {
  ...
  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-annot-'));
    fs.writeFileSync(
  ...
  it('getTools() keeps annotations, incl. lattice_sensor_explore whose description is rebuilt', () => {
    process.env[ENV] = ALL_TOOLS;
    ...
    const explore = got.find((t) => t.name === 'lattice_sensor_explore');
