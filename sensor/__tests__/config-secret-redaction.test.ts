/**
 * #383 — LatticeSensor indexes config KEYS but must never surface config VALUES.
 *
 ...
 * passwords, API keys, JDBC URLs with embedded creds). LatticeSensor must surface
 * the KEY and never the value — not in node metadata (docstring/signature),
 * not via `lattice_sensor_explore`'s verbatim source dump, and not via
 * `lattice_sensor_node` `includeCode`. An agent that genuinely needs a value can
 * read the file itself (a deliberate pull); LatticeSensor must never volunteer it.
 */
...
describe('config secret redaction (#383)', () => {
  ...
  it('lattice_sensor_explore surfaces the config key but NEVER the secret value', async () => {
  ...
  it('lattice_sensor_node includeCode returns the key, not the secret value', async () => {
    const res = await handler.execute('lattice_sensor_node', {
