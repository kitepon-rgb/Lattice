/**
 * lattice_sensor_node FILE READ mode: a `file` with no `symbol` reads that file like
...
describe('lattice_sensor_node file-view (Read replacement)', () => {
  let dir: string;
  ...
  beforeEach(async () => {
    ...
    cg = LatticeSensor.initSync(dir, { config: { include: ['**/*.ts', '**/*.properties'], exclude: [] } });
    await cg.indexAll();
  ...
  const text = async (args: Record<string, unknown>): Promise<string> =>
    (await h.execute('lattice_sensor_node', args)).content.map((c) => c.text).join('\n');
