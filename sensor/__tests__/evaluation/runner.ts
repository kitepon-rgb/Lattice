...
import { LatticeSensor } from '../../src/index.js';
...
if (!fs.existsSync(path.join(resolvedPath, '.lattice/sensor', 'sensor.db'))) {
  console.error(`No .lattice/sensor/sensor.db found at ${resolvedPath}`);
  process.exit(1);
}

let latticeSensorSha = 'unknown';
try {
  latticeSensorSha = execSync('git rev-parse --short HEAD', { encoding: 'utf-8' }).trim();
} catch {}

console.log(`\nLatticeSensor Eval — ${path.basename(resolvedPath)}`);
console.log(`Codebase: ${resolvedPath}`);
console.log(`Commit:   ${latticeSensorSha}`);
console.log(`Cases:    ${testCases.length}`);
console.log('');

async function run() {
  const cg = LatticeSensor.openSync(resolvedPath);
