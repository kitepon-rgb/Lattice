/**
 * Tests for the `latticeSensor version` affordances.
 *
...
describe('latticeSensor version affordances', () => {
  for (const spelling of ['version', '-v', '-version', '--version', '-V']) {
    it(`\`latticeSensor ${spelling}\` prints exactly the package version`, () => {
  ...
  it('`lattice-sensor help` prints usage and the command list', () => {
    const out = run(['help']);
    expect(out).toContain('Usage: lattice-sensor');
  ...
  it('a trailing `-v` is still the subcommand\'s --verbose, not the version intercept', () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-version-test-'));
    let combined = '';
    try {
