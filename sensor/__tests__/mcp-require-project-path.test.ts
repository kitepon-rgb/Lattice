 ...
 * started outside any repo, or a monorepo root whose `.lattice/sensor/` indexes live
...
const exploreOf = (defs: { name: string; inputSchema: { required?: string[] } }[]) =>
  defs.find((t) => t.name === 'lattice_sensor_explore')!;
...
describe('No-default-project requires projectPath in the schema (#993)', () => {
  ...
  it('marks projectPath required on lattice_sensor_explore when no default project is loaded', () => {
  ...
  it('a missing projectPath with no default is still SUCCESS-shaped guidance, not isError', async () => {
    ...
    const res = await new ToolHandler(null).execute('lattice_sensor_explore', { query: 'anything' });
...
describe('A default project keeps projectPath OPTIONAL (#993)', () => {
  ...
  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-reqpath-'));
    fs.writeFileSync(
  ...
  it('a bare call (no projectPath) still falls back to the default project', async () => {
    const res = await new ToolHandler(cg).execute('lattice_sensor_explore', { query: 'processPayment' });
    expect(res.isError).toBeUndefined();
