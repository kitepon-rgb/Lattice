 ...
 * failed with a misleading "LatticeSensor not initialized. Run 'latticeSensor init'."
 *
...
describe('MCP project resolution via roots/list (issue #196)', () => {
  let cwdDir: string;     // where the server is launched — has NO .lattice/sensor
  let projectDir: string; // the real indexed project the client reports
  let child: ChildProcessWithoutNullStreams | null = null;

  beforeEach(() => {
    cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-mcp-cwd-'));
  ...
  it('resolves the project from the client roots/list when no rootUri is sent', async () => {
    const cg = await LatticeSensor.init(projectDir);
    cg.close();
    ...
    send(child, { jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'lattice_sensor_status', arguments: {} } });
    ...
    expect(text).not.toContain('No LatticeSensor project is loaded');
  }, 20000);
  ...
  it('returns an actionable error when there is no rootUri and no roots capability', async () => {
    ...
    expect(text).toContain('No LatticeSensor project is loaded');
