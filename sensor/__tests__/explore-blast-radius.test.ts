/**
 * lattice_sensor_explore blast-radius section.
 *
...
describe('lattice_sensor_explore — blast radius', () => {
  let testDir: string;
  let cg: LatticeSensor;
  let handler: ToolHandler;

  beforeEach(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-blast-'));
  ...
  it('lists dependents (locations only) and covering tests for an entry symbol', async () => {
    const res = await handler.execute('lattice_sensor_explore', { query: 'target' });
  ...
  it('surfaces tests that cover a symbol transitively through its callers (#1475)', async () => {
    const res = await handler.execute('lattice_sensor_explore', { query: 'deepHelper' });
    const text = res.content[0].text;
  ...
  it('states only what was measured when no test exists up the caller chain', async () => {
    const res = await handler.execute('lattice_sensor_explore', { query: 'untestedHelper' });
    const text = res.content[0].text;
  ...
  it('omits symbols that have no dependents from the blast radius', async () => {
    const res = await handler.execute('lattice_sensor_explore', { query: 'lonelyLeaf' });
    const text = res.content[0].text;
