...
// Writer child: real lattice-sensor pragmas + deferred checkpointing, grows the WAL
...
describe('WAL heal after killed sessions (#1431)', () => {
  ...
  beforeEach(() => {
    ...
    dbPath = path.join(dir, 'lattice-sensor.db');
...
describe('daemon observability for watchdog kills (#1431)', () => {
  it('derives watchdog progressPaths from the project root', () => {
    ...
    try {
      ...
      expect(progressPaths![0].endsWith(path.join('.lattice', 'sensor', 'sensor.db'))).toBe(true);
  ...
  it('stamps log chunks with an ISO-8601 timestamp', () => {
    ...
    expect(String(stampLogChunk('[LatticeSensor daemon] Listening.\n'))).toMatch(iso);
