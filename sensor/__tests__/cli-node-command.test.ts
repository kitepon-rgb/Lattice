/**
 * `latticeSensor node` argument handling (#1044).
 *
 * File-read mode (`latticeSensor node -f <file>`) carries no symbol name, but the
...
describe('latticeSensor node — argument handling (#1044)', () => {
  let tempDir: string;

  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-node-cmd-'));
...
describe('latticeSensor node — symbol pinned to a file includes the body (#1284)', () => {
  let tempDir: string;

  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-node-pin-'));
