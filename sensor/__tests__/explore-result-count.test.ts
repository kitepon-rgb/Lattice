/**
 * lattice_sensor_explore — the "Found N symbols across M files." header reflects the
...
describe('lattice_sensor_explore — curated result count (#1046)', () => {
  let testDir: string;
  let cg: LatticeSensor;
  let handler: ToolHandler;

  beforeEach(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-count-'));
  ...
  it('header file count equals the number of rendered source sections', async () => {
    const res = await handler.execute('lattice_sensor_explore', { query: 'publish status' });
    ...
    expect(text).not.toContain('lattice-sensor-explore-summary');
  });
});
