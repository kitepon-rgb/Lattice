...
function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-pr19-test-'));
}
...
// =============================================================================
// Graph Traversal 'both' Direction Fix
// (requires better-sqlite3 - will use LatticeSensor integration)
// =============================================================================
...
describe('Graph Traversal Both Direction', () => {
  ...
  it.skipIf(!HAS_SQLITE)('should traverse both directions from a node', async () => {
    const LatticeSensor = (await import('../src/index')).default;
...
describe('Database Layer Improvements', () => {
  ...
  it.skipIf(!HAS_SQLITE)('should support batch insert of unresolved refs', async () => {
    ...
    const dbPath = path.join(testDir, 'sensor.db');
...
`);
      ...
      // note) so lattice_sensor_node can hand back every overload and the agent never
...
describe('CLI uninit', () => {
  ...
  it.skipIf(!HAS_SQLITE)('should uninitialize a project via LatticeSensor.uninitialize()', async () => {
    const LatticeSensor = (await import('../src/index')).default;

    // Initialize
    const cg = LatticeSensor.initSync(testDir);
    expect(LatticeSensor.isInitialized(testDir)).toBe(true);

    // Uninitialize
    cg.uninitialize();

    // .lattice/sensor directory should be removed
    expect(LatticeSensor.isInitialized(testDir)).toBe(false);
  });
});
