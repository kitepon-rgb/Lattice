 ...
 * committed `lattice-sensor.json` at the repo root, so files that would otherwise be
 ...
 * map, the loader's validation/normalization/caching of `lattice-sensor.json`, and a
...
describe('custom extension → language mapping (#906)', () => {
  ...
  describe('loadExtensionOverrides (lattice-sensor.json)', () => {
    let dir: string;
    beforeEach(() => {
    ...
    it('returns an empty map when there is no lattice-sensor.json', () => {
  ...
  describe('indexAll honors lattice-sensor.json end-to-end', () => {
    let dir: string;
    beforeEach(() => {
    ...
    it('indexes a custom-extension file mapped to a supported language', async () => {
      write('lattice-sensor.json', JSON.stringify({ extensions: { '.foo': 'typescript' } }));
    ...
    it('does NOT index the same file without lattice-sensor.json (zero-config preserved)', async () => {
      write('widget.foo', SOURCE);
