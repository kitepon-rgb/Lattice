/**
 * Adaptive output budget for lattice_sensor_explore (#185).
 *
 ...
 */
describe('lattice_sensor_explore output respects the adaptive budget', () => {
  let testDir: string;
  let cg: LatticeSensor;
  let handler: ToolHandler;

  beforeAll(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-explore-budget-'));
  ...
  it('keeps total output under the small-project cap', async () => {
    const result = await handler.execute('lattice_sensor_explore', { query: 'Session method helper' });
  ...
  it('prefixes source lines with line numbers by default (cat -n style)', async () => {
    delete process.env.LATTICE_SENSOR_EXPLORE_LINENUMS;
    const result = await handler.execute('lattice_sensor_explore', { query: 'Session method helper' });
    const text = result.content?.[0]?.text ?? '';
  ...
  it('omits line numbers when LATTICE_SENSOR_EXPLORE_LINENUMS=0', async () => {
    process.env.LATTICE_SENSOR_EXPLORE_LINENUMS = '0';
    try {
      const result = await handler.execute('lattice_sensor_explore', { query: 'Session method helper' });
      const text = result.content?.[0]?.text ?? '';
