...
describe('MCP catch-up gate', () => {
  ...
  beforeEach(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-catchup-gate-'));
  ...
  it('awaits the gate before serving the first tool call', async () => {
    ...
    const res = await handler.execute('lattice_sensor_search', { query: 'survivor' });
  ...
  it('drops the gate after first await — second call does not re-wait', async () => {
    ...
    await handler.execute('lattice_sensor_search', { query: 'survivor' });
  ...
  it('catch-up reconciles a deleted file before the first tool call sees it', async () => {
    ...
    const res = await handler.execute('lattice_sensor_search', { query: 'deletedLater' });
    expect(res.isError).toBeFalsy();
  ...
  it('does not hang the first call when catch-up runs past the timeout (#905)', async () => {
    ...
    // in the background.
    const prev = process.env.LATTICE_SENSOR_CATCHUP_GATE_TIMEOUT_MS;
    process.env.LATTICE_SENSOR_CATCHUP_GATE_TIMEOUT_MS = '50';
    ...
    try {
      ...
      if (prev === undefined) delete process.env.LATTICE_SENSOR_CATCHUP_GATE_TIMEOUT_MS;
      else process.env.LATTICE_SENSOR_CATCHUP_GATE_TIMEOUT_MS = prev;
    }
  });

  it('LATTICE_SENSOR_CATCHUP_GATE_TIMEOUT_MS=0 restores the unbounded wait', async () => {
    const prev = process.env.LATTICE_SENSOR_CATCHUP_GATE_TIMEOUT_MS;
    process.env.LATTICE_SENSOR_CATCHUP_GATE_TIMEOUT_MS = '0';
    try {
      let gateResolved = false;
      const gate = new Promise<void>((resolve) => {
        setTimeout(() => { gateResolved = true; resolve(); }, 80);
