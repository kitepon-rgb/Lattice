...
function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-security-test-'));
}
...
describe('Symlink escape prevention (#527)', () => {
  ...
  // is the chokepoint both content-serving read sinks go through (lattice_sensor_node
  // includeCode + lattice_sensor_explore source rendering), so it must resolve
  ...
  it('end-to-end: getCode never serves an out-of-root file reached via a dir symlink', async () => {
    ...
    const cg = LatticeSensor.initSync(root, { config: { include: ['**/*.ts'], exclude: [] } });
    try {
      await cg.indexAll();
...
describe('MCP Input Validation', () => {
  ...
  it('should reject non-string query in lattice_sensor_search', async () => {
    const result = await handler.execute('lattice_sensor_search', { query: null });
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain('non-empty string');
  });

  it('should reject empty string query in lattice_sensor_search', async () => {
    const result = await handler.execute('lattice_sensor_search', { query: '' });
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain('non-empty string');
  });

  it('should accept valid query in lattice_sensor_search', async () => {
    const result = await handler.execute('lattice_sensor_search', { query: 'example' });
    expect(result.isError).toBeFalsy();
  });

  it('should clamp limit to valid range in lattice_sensor_search', async () => {
    // Extremely large limit should still work (clamped to 100)
    const result = await handler.execute('lattice_sensor_search', { query: 'example', limit: 999999 });
    expect(result.isError).toBeFalsy();
  });

  it('should reject non-string symbol in lattice_sensor_callers', async () => {
    const result = await handler.execute('lattice_sensor_callers', { symbol: 123 });
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain('non-empty string');
  });

  it('should reject non-string query in lattice_sensor_explore', async () => {
    const result = await handler.execute('lattice_sensor_explore', { query: undefined });
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain('non-empty string');
  });

  it('should truncate oversized tool output', async () => {
    // Force a huge result set through lattice_sensor_search; the response must be
    ...
    const fakeHandler = new ToolHandler(fakeCg as unknown as LatticeSensor);

    const result = await fakeHandler.execute('lattice_sensor_search', { query: 'x' });

    expect(result.isError).toBeFalsy();
    expect(result.content[0].text).toContain('... (output truncated)');
  });

  it('should reject non-string symbol in lattice_sensor_impact', async () => {
    const result = await handler.execute('lattice_sensor_impact', { symbol: [] });
    expect(result.isError).toBe(true);
  });

  it('should reject non-string symbol in lattice_sensor_node', async () => {
    const result = await handler.execute('lattice_sensor_node', { symbol: false });
    expect(result.isError).toBe(true);
  });

  it('should reject non-string symbol in lattice_sensor_callees', async () => {
    const result = await handler.execute('lattice_sensor_callees', { symbol: {} });
    expect(result.isError).toBe(true);
  });

  it('should handle NaN limit gracefully', async () => {
    const result = await handler.execute('lattice_sensor_search', { query: 'example', limit: 'abc' });
    expect(result.isError).toBeFalsy();
  });

  it('should handle negative limit gracefully', async () => {
    const result = await handler.execute('lattice_sensor_search', { query: 'example', limit: -5 });
    expect(result.isError).toBeFalsy();
  });

  // #230: getLatticeSensor must reject a sensitive system directory passed as
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    async () => {
      const result = await handler.execute('lattice_sensor_search', {
        query: 'example',
        projectPath: '/etc',
      });
      expect(result.isError).toBe(true);
