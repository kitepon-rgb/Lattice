 ...
 *
 * lattice_sensor_node / lattice_sensor_explore read CURRENT bytes from disk but slice
...
import { ToolHandler, __setLoadLatticeSensorForTests } from '../src/mcp/tools';
 ...
 * render and lattice_sensor_node's whole-file stale fallback does NOT fit. */
...
describe('MCP stale-slice guard (#1474)', () => {
  ...
  let cgFixture: LatticeSensor;
  let cgOther: LatticeSensor;
  let handler: ToolHandler;

  beforeEach(async () => {
    fixtureDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-stale-slice-fx-'));
    otherDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-stale-slice-other-'));
    ...
    cgFixture = LatticeSensor.initSync(fixtureDir, { config: { include: ['**/*.ts'], exclude: [] } });
    await cgFixture.indexAll();
    cgOther = LatticeSensor.initSync(otherDir, { config: { include: ['**/*.ts'], exclude: [] } });
    ...
    __setLoadLatticeSensorForTests(LatticeSensor);
    handler = new ToolHandler(cgOther);
  });

  afterEach(() => {
    __setLoadLatticeSensorForTests(null);
  ...
  it('lattice_sensor_node never serves another symbol\'s body from a drifted file (cross-project)', async () => {
    shiftBigFile();
    const result = await handler.execute('lattice_sensor_node', {
      symbol: 'orchestrate',
      includeCode: true,
  ...
  it('lattice_sensor_node serves the full CURRENT source of a small drifted file (Read-parity fallback)', async () => {
  ...
  it('lattice_sensor_explore omits (never mis-slices) a big drifted file and flags line refs', async () => {
    shiftBigFile();
    const result = await handler.execute('lattice_sensor_explore', {
  ...
  it('re-syncing the project restores normal output with no drift markers', async () => {
    ...
    try {
      const result = await freshHandler.execute('lattice_sensor_node', {
        symbol: 'orchestrate',
        includeCode: true,
        projectPath: fixtureDir,
      });
  ...
  it('the guard also fires on the default project when no watcher is running', async () => {
    ...
    try {
      const result = await direct.execute('lattice_sensor_node', {
        symbol: 'orchestrate',
        includeCode: true,
      });
      const text = getText(result);
