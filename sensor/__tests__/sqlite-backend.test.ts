 ...
 * DatabaseConnection / LatticeSensor report it and come up in WAL.
 */
...
describe('DatabaseConnection — backend reporting', () => {
  ...
  beforeEach(() => {
    dir = fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-backend-'));
  });
  ...
  it('LatticeSensor.getBackend() delegates to the underlying DatabaseConnection', async () => {
