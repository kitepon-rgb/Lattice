 ...
 *
 * Pinned because the lookup vocabulary is what makes latticeSensor useful
...
function tmpRoot(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-symbol-lookup-'));
}
...
describe.skipIf(!HAS_SQLITE)('matchesSymbol — module-qualified lookups (#173)', () => {
  ...
  beforeEach(async () => {
    ...
    cg = LatticeSensor.initSync(projectRoot, {
  ...
  it('lattice_sensor_node with a `file` hint pins an overloaded name to that file', async () => {
...
describe.skipIf(!HAS_SQLITE)('matchesSymbol — dotted lookups (regression for #173 fix)', () => {
  ...
  it('lattice_sensor_node on an ambiguous bare name returns ALL overloads with bodies (no guess)', async () => {
    ...
    const res = await handler.execute('lattice_sensor_node', { symbol: 'request', includeCode: true });
    const text = res.content?.[0]?.text ?? '';
