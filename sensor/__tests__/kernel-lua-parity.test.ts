 ...
 * migration).
 *
 * Asserts the native walker (lattice-sensor-kernel/src/lua.rs — one module, two
 ...
 * LATTICE_SENSOR_KERNEL_EXPECT=1 turns that into a failure.
 */
...
describe.skipIf(!kernelBuilt)('kernel Lua/Luau extraction parity', () => {
  ...
  it('torture.lua: requires, receiver QNs, visibility inversion, callee zoo, fn-refs', () => {
    ...
    // Top-level requires became imports — and so does the body-level one.
    // Upstream pinned the body-level require as a bare CALL; Lattice efc8299
    // supersedes that: the nested require emits the same import node + imports
    // ref as a top-level require and is claimed (no `calls require` residue).
    expect(result.nodes.some((n) => n.kind === 'import' && n.name === 'app.core')).toBe(true);
    expect(result.nodes.some((n) => n.kind === 'import' && n.name === 'app.lazy')).toBe(true);
