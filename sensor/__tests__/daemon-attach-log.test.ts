 ...
 * It's now gated behind LATTICE_SENSOR_MCP_LOG_ATTACH=1 — silent by default, opt-in
 * for debugging. Approach from #640 by @mturac.
 */
import { describe, it, expect, beforeEach, afterEach, vi } from 'vitest';
import { logAttachedDaemon } from '../src/mcp/proxy';

const hello = { pid: 4242, sensor: '9.9.9' } as any;
...
describe('daemon attach log gating (#618)', () => {
  ...
  afterEach(() => {
    spy.mockRestore();
    delete process.env.LATTICE_SENSOR_MCP_LOG_ATTACH;
  });
  ...
  it('logs the attach line only when LATTICE_SENSOR_MCP_LOG_ATTACH=1 (opt-in debug)', () => {
    process.env.LATTICE_SENSOR_MCP_LOG_ATTACH = '1';
    logAttachedDaemon('/tmp/cg.sock', hello);
