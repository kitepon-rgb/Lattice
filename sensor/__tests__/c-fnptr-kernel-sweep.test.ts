 ...
 * sweep and the kernel's `cfnptrScanFiles` (lattice-sensor-kernel/src/cfnptr.rs).
 * They must be record-identical, which this suite pins end-to-end: the same
 * adversarial project is indexed twice — LATTICE_SENSOR_KERNEL_CFNPTR toggled —
...
describe.runIf(nativeAvailable)('cFnPtr sweep: native vs JS differential', () => {
  ...
  afterEach(() => {
    delete process.env.LATTICE_SENSOR_KERNEL_CFNPTR;
...
`);
  ...
  it('indexes to identical fn-pointer-dispatch edges with the sweep native vs JS', async () => {
    writeFixture();
    process.env.LATTICE_SENSOR_KERNEL_CFNPTR = '0';
