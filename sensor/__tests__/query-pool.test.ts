 ...
 * connection and runs lattice_sensor_explore) is validated separately against a real
...
describe('QueryPool', () => {
  it('dispatches a call and returns the worker result', async () => {
    ...
    const res = await pool.run('lattice_sensor_explore', { query: 'q' });
    expect(res.content[0].text).toBe('r:lattice_sensor_explore');
    await pool.destroy();
  });
  ...
  it('runs N concurrent calls in parallel (not serialized)', async () => {
    ...
    const calls = Promise.all(Array.from({ length: 5 }, (_, i) => pool.run('lattice_sensor_search', { i })));
  ...
  it('does not spawn the whole pool for a single call (pending-aware growth)', async () => {
    ...
    await pool.run('lattice_sensor_node', { symbol: 's' });
  ...
  it('recovers from a worker crash: retries the in-flight call and respawns', async () => {
    ...
    const again = await pool.run('lattice_sensor_node', { symbol: 's' });
  ...
  it('fails a poison call gracefully without wedging the pool', async () => {
    ...
    const poison = (m: CallMsg) => m.toolName === 'lattice_sensor_explore';
    const pool = new QueryPool({
      root: '/x', size: 3, maxRetries: 1,
      createWorker: () => new FakeWorker((m) => (poison(m) ? { crash: true } : { result: ok(`ok:${m.id}`) })),
    });
    const bad = await pool.run('lattice_sensor_explore', { query: 'boom' });
    expect(bad.isError).toBe(true); // graceful, after retries
    const good = await pool.run('lattice_sensor_search', { query: 'fine' });
  ...
  it('destroy settles outstanding calls instead of hanging', async () => {
    ...
    const pending = pool.run('lattice_sensor_explore', { query: 'q' });
    await sleep(5);
    await pool.destroy();
  ...
  it('is not `ready` until a worker completes its cold start (#662 first-call stall)', async () => {
    ...
    const res = await pool.run('lattice_sensor_status', {});
    expect(res.content[0].text).toBe('r:lattice_sensor_status');
    await pool.destroy();
