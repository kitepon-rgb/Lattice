 ...
 *
 * A long-lived process (the MCP daemon) opens `.lattice/sensor/sensor.db` and
 * holds the file descriptor for its whole life. If `.lattice/sensor/` is removed and
 * recreated AT THE SAME PATH while it's running — `git worktree remove <p>` then
 * `git worktree add <p>` + `latticeSensor init`, or `rm -rf .lattice/sensor` + re-init —
...
import { getLatticeSensorDir } from '../src/directory';
...
describe('DatabaseConnection.isReplacedOnDisk (issue #925)', () => {
  ...
  beforeEach(() => {
    ...
    dbPath = path.join(dir, 'sensor.db');
...
describe('LatticeSensor.reopenIfReplaced (issue #925)', () => {
  let root: string;
  ...
  posixOnly('heals a held connection after the index is removed and recreated at the same path', async () => {
    ...
    // Simulate `git worktree remove` + re-add (or rm -rf .lattice/sensor + init):
    // a NEW index inode at the same path, carrying a renamed symbol, written by
    // a separate instance (mirrors a fresh `latticeSensor init` process).
    ...
    const fresh = LatticeSensor.initSync(root);
