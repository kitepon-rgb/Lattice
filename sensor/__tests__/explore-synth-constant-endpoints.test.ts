/**
 * Regression: lattice_sensor_explore must SURFACE a synthesized edge whose endpoints are
...
// Assertions read RAW lattice_sensor_explore output; managed offload would replace it. Disable
...
beforeAll(() => { _prevOffloadDisable = process.env.LATTICE_SENSOR_OFFLOAD_DISABLE; process.env.LATTICE_SENSOR_OFFLOAD_DISABLE = '1'; });
afterAll(() => {
  if (_prevOffloadDisable === undefined) delete process.env.LATTICE_SENSOR_OFFLOAD_DISABLE;
  else process.env.LATTICE_SENSOR_OFFLOAD_DISABLE = _prevOffloadDisable;
});

describe('lattice_sensor_explore — synthesized constant→constant edges surface on small repos', () => {
  let dir: string;
...
`
    ...
    cg = LatticeSensor.initSync(dir, { config: { include: ['**/*.ts'], exclude: [] } });
    await cg.indexAll();
    ...
    const res = await handler.execute('lattice_sensor_explore', { query: 'outerThunk innerThunk' });
