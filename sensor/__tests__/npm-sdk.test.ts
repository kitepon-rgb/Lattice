 ...
 * `require("@colbymchenry/latticeSensor")` on their OWN Node by re-exporting the
 * compiled library that ships inside the per-platform optionalDependency
 * (@colbymchenry/lattice-sensor-<target>/lib/dist/index.js).
 *
...
// SDK's `require.resolve('@colbymchenry/lattice-sensor-<target>/...')` walks to it.
function makeConsumer(): { root: string; mainPkg: string } {
  const root = mkTmp('consumer');
  const mainPkg = path.join(root, 'node_modules', '@colbymchenry', 'latticeSensor');
  fs.mkdirSync(mainPkg, { recursive: true });
  fs.copyFileSync(SDK_SRC, path.join(mainPkg, 'npm-sdk.js'));
  fs.writeFileSync(
    path.join(mainPkg, 'package.json'),
    JSON.stringify({ name: '@colbymchenry/latticeSensor', version: VERSION, main: 'npm-sdk.js' }) + '\n'
  );
  return { root, mainPkg };
}
...
function writeFakeLib(libDistDir: string, sentinel: string): void {
  ...
  fs.writeFileSync(
    ...
    `module.exports = { SENTINEL: ${JSON.stringify(sentinel)}, LatticeSensor: function LatticeSensor() {} };\n`
  );
}

function installPlatformPackage(root: string, sentinel: string): void {
  const pkgRoot = path.join(root, 'node_modules', '@colbymchenry', `lattice-sensor-${target}`);
  writeFakeLib(path.join(pkgRoot, 'lib', 'dist'), sentinel);
  fs.writeFileSync(
    path.join(pkgRoot, 'package.json'),
    JSON.stringify({ name: `@colbymchenry/lattice-sensor-${target}`, version: VERSION }) + '\n'
  );
}
...
function requireSdk(mainPkg: string, env: Record<string, string> = {}) {
  const code =
    ...
    ` process.stdout.write(JSON.stringify({ sentinel: m.SENTINEL, cg: typeof m.LatticeSensor })); }` +
...
describe('npm-sdk programmatic entry', () => {
  it('re-exports the installed platform bundle library', () => {
    ...
    const r = requireSdk(mainPkg, { LATTICE_SENSOR_INSTALL_DIR: path.join(root, '.empty-cache') });
  ...
  it('falls back to a self-healed cache bundle when the optional dep is absent', () => {
    ...
    const r = requireSdk(mainPkg, { LATTICE_SENSOR_INSTALL_DIR: cacheDir });
    expect(r.status).toBe(0);
