 ...
 * directory to index embedded repos there is OPT-IN via `lattice-sensor.json`
...
describe('multi-repo workspaces (#514) + .gitignore-respect default (#970, #976)', () => {
  ...
  /** Drop a `lattice-sensor.json` at the workspace root. */
  const writeConfig = (obj: unknown) =>
    fs.writeFileSync(path.join(ws, 'lattice-sensor.json'),
  ...
  describe('opt-in: lattice-sensor.json includeIgnored re-includes a gitignored dir (#622, #699)', () => {
    ...
    it('sync picks up a change inside an opted-in gitignored embedded repo', async () => {
      ...
      const cg = LatticeSensor.initSync(ws, { config: { include: ['**/*.ts'], exclude: [] } });
      try {
        await cg.indexAll();
  ...
  describe('discovery/classifier machinery (exercised under opt-in)', () => {
    ...
    it('still indexes UNTRACKED embedded repos by default (#193 regression)', () => {
      ...
      // No lattice-sensor.json: the untracked path is unaffected by the opt-in gate.
      const files = scanDirectory(ws);
    ...
    it('buildScopeIgnore: indexed root is itself a gitignored subdir of an enclosing repo (#936)', () => {
      ...
      // silently stalled until a manual `latticeSensor sync`.
