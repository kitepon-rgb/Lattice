...
describe('parseClientHelloLine', () => {
  it('parses a well-formed client-hello', () => {
    expect(parseClientHelloLine('{"lattice_sensor_client":1,"pid":1234,"hostPid":56}'))
      .toEqual({ pid: 1234, hostPid: 56 });
  });

  it('accepts a null host pid and a missing host pid', () => {
    expect(parseClientHelloLine('{"lattice_sensor_client":1,"pid":1234,"hostPid":null}'))
      .toEqual({ pid: 1234, hostPid: null });
    expect(parseClientHelloLine('{"lattice_sensor_client":1,"pid":1234}'))
      .toEqual({ pid: 1234, hostPid: null });
  });
  ...
  it('rejects a wrong-typed marker, a non-numeric pid, and a non-integer marker', () => {
    expect(parseClientHelloLine('{"lattice_sensor_client":true,"pid":1}')).toBeNull();
    expect(parseClientHelloLine('{"lattice_sensor_client":2,"pid":1}')).toBeNull();
    expect(parseClientHelloLine('{"lattice_sensor_client":1,"pid":"1"}')).toBeNull();
  });
...
describe('Daemon.reapDeadClients', () => {
  ...
  const makeDaemon = () => new Daemon('/tmp/lattice-sensor-reap-unit-test', { idleTimeoutMs: 0 }) as any;
...
describe('Daemon.backstopShouldExit', () => {
  ...
  const makeDaemon = () => {
    const d = new Daemon('/tmp/lattice-sensor-backstop-unit-test', { idleTimeoutMs: 0, maxIdleMs: 1000 }) as any;
