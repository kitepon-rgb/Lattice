...
function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-installer-test-'));
}
...
describe('Installer Config Writer', () => {
  ...
  describe('readJsonFile error handling', () => {
    it('should return empty object for non-existent file', () => {
      ...
      expect(content.mcpServers.lattice/sensor).toBeDefined();
    });
    ...
    it('should handle corrupted JSON by creating backup', () => {
      ...
      // New file should be valid JSON with latticeSensor config
      const content = JSON.parse(fs.readFileSync(mcpJson, 'utf-8'));
      expect(content.mcpServers.lattice/sensor).toBeDefined();
    ...
    it('should preserve existing valid config when adding latticeSensor', () => {
      const mcpJson = path.join(tempDir, '.mcp.json');
