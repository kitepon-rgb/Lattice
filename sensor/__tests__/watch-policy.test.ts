...
describe('watchDisabledReason', () => {
  it('returns a reason when LATTICE_SENSOR_NO_WATCH=1', () => {
    const reason = watchDisabledReason('/home/me/project', {
      env: { LATTICE_SENSOR_NO_WATCH: '1' },
      isWsl: false,
    });
    expect(reason).toBeTruthy();
    expect(reason).toMatch(/LATTICE_SENSOR_NO_WATCH/);
  });
  ...
  it('LATTICE_SENSOR_FORCE_WATCH=1 overrides WSL auto-detect', () => {
    const reason = watchDisabledReason('/mnt/d/code/project', {
      env: { LATTICE_SENSOR_FORCE_WATCH: '1' },
      isWsl: true,
    });
    expect(reason).toBeNull();
  });

  it('LATTICE_SENSOR_NO_WATCH wins over LATTICE_SENSOR_FORCE_WATCH', () => {
    const reason = watchDisabledReason('/home/me/project', {
      env: { LATTICE_SENSOR_NO_WATCH: '1', LATTICE_SENSOR_FORCE_WATCH: '1' },
      isWsl: false,
    });
    expect(reason).toBeTruthy();
  });
...
describe('FileWatcher honors the watch policy', () => {
  ...
  afterEach(() => {
    delete process.env.LATTICE_SENSOR_NO_WATCH;
    if (testDir && fs.existsSync(testDir)) {
      fs.rmSync(testDir, { recursive: true, force: true });
    }
  });

  it('does not start when LATTICE_SENSOR_NO_WATCH=1', () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-nowatch-'));
