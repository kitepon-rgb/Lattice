/**
 * lattice_sensor_explore — multi-term corroboration tier (cross-layer monorepo ranking).
 *
...
describe('lattice_sensor_explore — multi-term corroboration tier', () => {
  let testDir: string;
  let cg: LatticeSensor;
  let handler: ToolHandler;

  beforeEach(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-corrob-'));
  ...
  it('sources the corroborated backend file alongside a denser frontend cluster in a multi-layer repo', async () => {
    const res = await handler.execute('lattice_sensor_explore', { query: 'item service' });
  ...
  it('still leads with the backend file when the query names its symbol directly', async () => {
    ...
    const res = await handler.execute('lattice_sensor_explore', { query: 'DataService read load' });
    const text = res.content[0].text;
    const sourced = sourcedFiles(text);
    expect(sourced).toContain('api/item/service.ts');
