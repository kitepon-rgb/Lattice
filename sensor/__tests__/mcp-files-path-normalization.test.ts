/**
 * lattice_sensor_files path-filter normalization (#426)
 *
...
describe('lattice_sensor_files path normalization', () => {
  let tempDir: string;
  let cg: LatticeSensor;
  let handler: ToolHandler;

  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lattice-sensor-files-paths-'));
  ...
  async function listed(pathFilter: string | undefined): Promise<string> {
    const result = await handler.execute('lattice_sensor_files', {
