...
//   LATTICE_SENSOR_NO_DOWNLOAD=1     disable the network fallback (print guidance)
//   LATTICE_SENSOR_INSTALL_DIR=DIR   cache location (default: ~/.lattice-sensor)
//   LATTICE_SENSOR_DOWNLOAD_BASE=URL release-download base (for mirrors/air-gapped)
//
...
async function main() {
  ...
  if (!env.LATTICE_SENSOR_HOST_PPID) env.LATTICE_SENSOR_HOST_PPID = String(process.ppid);
  var res = childProcess.spawnSync(resolved.command, resolved.args, { stdio: 'inherit', windowsHide: true, env: env });
  if (res.error) {
    process.stderr.write('lattice-sensor: ' + res.error.message + '\n');
    process.exit(1);
  }
...
function resolveInstalledBundle() {
  try {
    if (isWindows) {
      ...
      var entry = require.resolve(pkg + '/lib/dist/bin/lattice-sensor.js');
      return { command: nodeExe, args: liftoff(entry) };
    }
    return { command: require.resolve(pkg + '/bin/lattice-sensor'), args: process.argv.slice(2) };
  } catch (e) {
    return null;
  }
}
...
function launcherIn(dir) {
  if (isWindows) {
    ...
    var entry = path.join(dir, 'lib', 'dist', 'bin', 'lattice-sensor.js');
    if (fs.existsSync(nodeExe) && fs.existsSync(entry)) {
      return { command: nodeExe, args: liftoff(entry) };
    }
  } else {
    var launcher = path.join(dir, 'bin', 'lattice-sensor');
...
// avoid the Zone OOM on Node >= 22 (issues #293/#298). The unix bin/lattice-sensor
...
async function selfHealBundle() {
  ...
  var bundlesDir = path.join(process.env.LATTICE_SENSOR_INSTALL_DIR || path.join(os.homedir(), '.lattice-sensor'), 'bundles');
  var dest = path.join(bundlesDir, target + '-' + version);

  // Already downloaded by a previous run? Use it even when downloads are
  // disabled — LATTICE_SENSOR_NO_DOWNLOAD blocks fetching, not a cached bundle.
  var cached = launcherIn(dest);
  if (cached) { pruneOldBundles(bundlesDir, dest); return cached; }

  if (process.env.LATTICE_SENSOR_NO_DOWNLOAD) {
    fail('the network fallback is disabled (LATTICE_SENSOR_NO_DOWNLOAD is set).');
  }

  var asset = 'lattice-sensor-' + target + (isWindows ? '.zip' : '.tar.gz');
  var base = process.env.LATTICE_SENSOR_DOWNLOAD_BASE || ('https://github.com/' + REPO + '/releases/download');
  var url = base + '/v' + version + '/' + asset;

  process.stderr.write(
    'lattice-sensor: platform bundle missing (registry did not provide ' + pkg + ').\n' +
    'lattice-sensor: downloading ' + asset + ' from GitHub Releases (' + version + ')...\n'
  );

  // Stage inside bundlesDir so the final rename is on the same filesystem (atomic,
  // no EXDEV across tmpfs). Strip the archive's top-level lattice-sensor-<target>/ dir.
  ...
  process.stderr.write('lattice-sensor: bundle ready.\n');
  return ready;
}
...
function download(url, dest, redirectsLeft) {
  return new Promise(function (resolve, reject) {
    ...
    var req = https.get(url, { headers: { 'User-Agent': 'lattice-sensor-npm-shim' }, timeout: 30000 }, function (res) {
...
async function verifyChecksum(archivePath, asset, base, version) {
  ...
  process.stderr.write('lattice-sensor: checksum verified.\n');
}
...
function fail(reason) {
  process.stderr.write(
    'lattice-sensor: no prebuilt bundle for ' + target + '.\n' +
    (reason ? 'lattice-sensor: ' + reason + '\n' : '') +
