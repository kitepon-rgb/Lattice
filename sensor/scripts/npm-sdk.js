'use strict';
//
// Programmatic / embedded SDK entry for @kitepon-rgb/Lattice (issue #354).
//
...
// API — `require("@kitepon-rgb/Lattice")` returning the Lattice sensor class et al.
//
// The compiled library + its production dependencies (web-tree-sitter,
// tree-sitter-wasms, …) ship INSIDE the per-platform bundle, at
//   @kitepon-rgb/Lattice-<platform>-<arch>/lib/dist/index.js
...
function resolveLibrary() {
  ...
  throw new Error(
    'lattice-sensor: the programmatic API is unavailable because the platform bundle\n' +
    ...
    '      npm i @kitepon-rgb/Lattice --registry=https://registry.npmjs.org\n' +
    '  - or run the CLI once (e.g. `npx @kitepon-rgb/Lattice status`) to\n' +
    '    self-heal the bundle into ~/.lattice-sensor, then require() will find it.'
  );
}
...
function cachedLibrary() {
  try {
    ...
    var base = process.env.LATTICE_SENSOR_INSTALL_DIR || path.join(os.homedir(), '.lattice-sensor');
