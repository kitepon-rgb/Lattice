#!/usr/bin/env bash
# Build the current branch and link it as the global `lattice-sensor` for
...
if [ "${1:-}" = "--undo" ]; then
  ...
  npm install -g "${PKG}"
  echo "done: global lattice-sensor -> $(command -v lattice-sensor)"
  exit 0
fi
...
LINKED=$(command -v lattice-sensor || echo "(not on PATH)")
echo
echo "✓ global lattice-sensor now points to this branch"
echo "  binary:  ${LINKED}"
echo "  branch:  ${BRANCH}"
