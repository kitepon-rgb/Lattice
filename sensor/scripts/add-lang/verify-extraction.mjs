#!/usr/bin/env node
// Sanity-check that lattice-sensor extracted REAL symbols (not just file/import nodes)
...
// Reads `lattice-sensor status <repo> --json` using whatever lattice-sensor is on PATH,
...
let status;
try {
  const out = execFileSync('lattice-sensor', ['status', repo, '--json'], { encoding: 'utf8' });
  status = JSON.parse(out);
} catch (e) {
  console.error(`[verify] could not read lattice-sensor status for ${repo}: ${e.message}`);
  process.exit(2);
}

// Kinds that prove the extractor mapped AST node types (everything except
// 'file' and 'import', which lattice-sensor creates structurally for any language).
