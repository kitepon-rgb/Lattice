...
#   clone -> wipe+index (with the lattice-sensor on PATH) -> verify extraction ->
#   with/without retrieval A/B (reuses scripts/agent-eval/run-all.sh).
#
# Assumes the lattice-sensor dev build is already built + linked on PATH — the skill
...
# Env:   CORPUS   corpus dir (default /tmp/lattice-sensor-corpus, shared with agent-eval)
set -uo pipefail
...
command -v lattice-sensor >/dev/null || { echo "no lattice-sensor on PATH (build + ./scripts/local-install.sh first)"; exit 1; }

echo "==================== add-lang bench: $NAME ($LANG_TOKEN) ===================="
echo "lattice-sensor: $(command -v lattice-sensor) -> $(lattice-sensor --version 2>/dev/null || echo '?')"
