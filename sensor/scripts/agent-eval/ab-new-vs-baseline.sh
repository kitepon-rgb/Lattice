#!/usr/bin/env bash
# A/B a lattice-sensor retrieval/steering change: the NEW build (current HEAD) vs a
# BASELINE build (a git ref) — BOTH with lattice-sensor attached — on the same
# implementation task, measuring how many Read vs lattice-sensor calls the agent
...
# session): each arm PRE-WARMS a persistent lattice-sensor daemon for its target so
# claude connects to an already-bound, index-loaded daemon instantly — before
# the agent's first turn — and SKIPS lattice-sensor's startup re-exec via
# LATTICE_SENSOR_WASM_RELAUNCHED=1. Without this, on a multi-step task the agent
# dives into Read/grep before lattice-sensor finishes its ~2-3s startup (worse under
# the CPU contention of a nested run) and runs with NO lattice-sensor.
#
# Gotcha: claude's `system/init` snapshot can read status:"pending" / 0 tools
# even when the server then connects fine — judge by ACTUAL lattice-sensor usage in
...
prewarm() { # target — spawn a persistent daemon (current $BIN) and wait for its socket
  ...
  node -e 'const fs=require("fs");let n=0;const t=setInterval(()=>{if(fs.existsSync(process.argv[1]+"/.lattice/sensor/daemon.sock")){clearInterval(t);process.exit(0)}if(n++>150){clearInterval(t);process.exit(1)}},100)' "$1" \
    && echo "  daemon warm: $1" || echo "  WARN: daemon never bound for $1 (arm may run without lattice-sensor)"
}
...
echo "###### (especially Read vs mcp__lattice_sensor__*). Full logs in: $OUT"
