...
//
// Mirrors the daemon's reality: ONE Lattice sensor + ONE ToolHandler (as MCPEngine
// shares across all sessions), then fires N concurrent lattice_sensor_explore calls
// and measures:
...
function runOne(i) {
  ...
  const work = handler
    .execute('lattice_sensor_explore', { query: q })
