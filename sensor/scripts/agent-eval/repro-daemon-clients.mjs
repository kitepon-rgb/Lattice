...
//
// Spawns N real `lattice-sensor serve --mcp --path <repo>` processes (each becomes a
...
// concurrent wave of lattice_sensor_explore tools/call across all N and measures
...
const CLI = resolve('dist/bin/lattice-sensor.js');
...
function makeClient(id) {
  const child = spawn('node', [CLI, 'serve', '--mcp', '--path', repo], {
    env: { ...process.env, LATTICE_SENSOR_TELEMETRY: '0', DO_NOT_TRACK: '1', LATTICE_SENSOR_MCP_LOG_ATTACH: '0' },
...
if (WARM) {
  ...
  const r = await clients[0].request('tools/call', { name: 'lattice_sensor_explore', arguments: { query: QUERIES[0] } }, 'warm-0', 120000);
...
const results = await Promise.all(clients.map((c, i) => {
  ...
  return c.request('tools/call', { name: 'lattice_sensor_explore', arguments: { query: QUERIES[i % QUERIES.length] } }, `call-${c.id}`, TIMEOUT_MS)
    .then((m) => ({
      id: c.id,
