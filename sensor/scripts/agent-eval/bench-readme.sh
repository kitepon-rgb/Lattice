#!/usr/bin/env bash
# Re-run the README "Benchmark Results" A/B (with vs without lattice-sensor) on the
...
# $CORPUS (default /tmp/lattice-sensor-corpus) by the build under test.
set -uo pipefail
H="$(cd "$(dirname "$0")" && pwd)"
C="${CORPUS:-/tmp/lattice-sensor-corpus}"
RUNS="${RUNS:-4}"
ROWS=(
