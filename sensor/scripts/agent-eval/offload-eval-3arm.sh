...
#   ARM offload : lattice-sensor attached, managed offload ON  (per-run AI usage log)
#   ARM raw     : lattice-sensor attached, LATTICE_SENSOR_OFFLOAD_DISABLE=1 (raw source)
#   ARM nocg    : no lattice-sensor (empty MCP config) -> Read/Grep baseline
...
run() { # arm rep mcp-config usage-log-or-dash
  ...
  # measures the retrieval mode, not whether Sonnet decides to spawn a lattice-sensor-blind
...
printf '{"mcpServers":{"lattice-sensor":{"command":"env","args":["LATTICE_SENSOR_WASM_RELAUNCHED=1","LATTICE_SENSOR_OFFLOAD_USAGE_LOG=%s","node","%s","serve","--mcp","--path","%s"]}}}' "$USAGE" "$BIN" "$TARGET" > "$CFG_OFF"
printf '{"mcpServers":{"lattice-sensor":{"command":"env","args":["LATTICE_SENSOR_WASM_RELAUNCHED=1","LATTICE_SENSOR_OFFLOAD_DISABLE=1","node","%s","serve","--mcp","--path","%s"]}}}' "$BIN" "$TARGET" > "$CFG_RAW"
