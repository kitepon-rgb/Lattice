...
// Front-loads lattice-sensor's structural answer for flow/impact/"how/where" prompts so the
...
try {
  process.env.LATTICE_SENSOR_OFFLOAD_DISABLE = '1'; // raw, accurate — never the unfixed offload
  process.env.LATTICE_SENSOR_TELEMETRY = '0'; process.env.DO_NOT_TRACK = '1';
  ...
  if (typeof Lattice sensor?.openSync !== 'function' || typeof ToolHandler !== 'function') process.exit(0);
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      const cg = Lattice sensor.openSync(found);
      const h = new ToolHandler(cg);
      const res = await h.execute('lattice_sensor_explore', { query: prompt });
  ...
  process.stdout.write(
    `## Lattice sensor structural context (auto-retrieved for this question)\n` +
    `The code graph was queried for your question; the relevant symbols, source, and call flow are below. ` +
    `Treat the quoted source as already read. If you need more, call lattice_sensor_explore with specific symbol names rather than grepping or reading files.\n\n` +
    text + '\n'
  );
