#!/usr/bin/env bash
# Headless Claude Code run against a repo with lattice-sensor MCP, capturing the
...
# Env: AGENT_EVAL_OUT (default /tmp/agent-eval), CG_BIN (lattice-sensor dist binary)
set -uo pipefail

REPO="$1"; LABEL="$2"; PROMPT="$3"
CG_BIN="${CG_BIN:-$(command -v lattice-sensor || echo /usr/local/bin/lattice-sensor)}"
