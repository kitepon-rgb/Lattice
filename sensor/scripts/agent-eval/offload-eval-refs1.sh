...
set -uo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"; ENGINE="$(cd "$HERE/../.." && pwd)"; BIN="$ENGINE/dist/bin/lattice-sensor.js"
...
# `raw` is a pseudo-style: lattice-sensor attached but the offload DISABLED (the ceiling —
# verbatim source, no reasoning model). Any other value is an offload style (plain|refs).
if [ "$STYLE" = "raw" ]; then
  DAEMON_ENV="LATTICE_SENSOR_OFFLOAD_DISABLE=1"
  printf '{"mcpServers":{"lattice-sensor":{"command":"env","args":["LATTICE_SENSOR_WASM_RELAUNCHED=1","LATTICE_SENSOR_OFFLOAD_DISABLE=1","node","%s","serve","--mcp","--path","%s"]}}}' \
    "$BIN" "$TARGET" > "$CFG"
  USAGE="-"
else
  DAEMON_ENV="LATTICE_SENSOR_OFFLOAD_STYLE=$STYLE LATTICE_SENSOR_OFFLOAD_USAGE_LOG=$USAGE"
  printf '{"mcpServers":{"lattice-sensor":{"command":"env","args":["LATTICE_SENSOR_WASM_RELAUNCHED=1","LATTICE_SENSOR_OFFLOAD_STYLE=%s","LATTICE_SENSOR_OFFLOAD_USAGE_LOG=%s","node","%s","serve","--mcp","--path","%s"]}}}' \
...
# salience, ignores lattice-sensor, and thrashes via Bash+Read, making the A/B noise).
