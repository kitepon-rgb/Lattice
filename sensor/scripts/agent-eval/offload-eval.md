# Lattice sensor AI offload — accuracy & adoption eval harness

Measures the managed **offload** (`lattice_sensor_explore` → reasoning model synthesis) and the
**front-load hook** (approach 1) against plain lattice-sensor and no-lattice-sensor, across repo sizes,
on **time · main-session tokens/cost · Lattice sensor-AI tokens/cost · accuracy**.
...
flow questions from memory, so the no-lattice-sensor baseline is dishonest. These four passed a no-tools
...
- **offload** — lattice-sensor + managed offload ON (requires `lattice-sensor login`); records AI tokens/credits via `LATTICE_SENSOR_OFFLOAD_USAGE_LOG`.
- **raw** — lattice-sensor, `LATTICE_SENSOR_OFFLOAD_DISABLE=1` (returns raw source).
- **nocg** — empty MCP config; Read/Grep baseline.
- **frontload** — lattice-sensor (offload-disabled) + a `UserPromptSubmit` hook (`offload-eval-hook.mjs`) that runs raw explore on the prompt and injects the result into context (approach 1).

## Run it

```bash
npm run build                       # the harness shells out to dist/
lattice-sensor login                     # only needed for the offload arm
...
- **Raw lattice-sensor is the efficiency win** — ~nocg accuracy, fewer reads, faster, no AI cost.
