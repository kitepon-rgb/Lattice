  ...
  for (const l of L) { let e; try { e = JSON.parse(l); } catch { continue; }
    if (e.type === 'assistant') {
      ...
      for (const b of (e.message?.content || [])) if (b.type === 'tool_use') {
        ...
        else if (/lattice-sensor/.test(n)) cg++;
      }
    }
    // MCP cold-start race: the headless agent fired before `lattice-sensor serve --mcp`
    // finished registering its tools, so early calls returned "No such tool
    // available" and the agent floundered into grep/Read. That measures Lattice sensor's
