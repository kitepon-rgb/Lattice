...
# onto lattice_sensor_node during implementation, not just for Q&A).
#
# When the agent Reads a SOURCE file, deny it and steer to lattice_sensor_node's
...
# A redirect to a file lattice-sensor hasn't indexed SELF-CORRECTS: the file-view
...
# Eval artifact only. The production version is an indexed-aware `lattice-sensor`
