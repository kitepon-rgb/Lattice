...
# lattice-sensor (explore/node) to understand before editing, does it still Read? Like
# ab-sufficiency.sh but copies+indexes a FRESH target per run (the agent mutates
# it), so runs don't see each other's edits.
#
# WITH lattice-sensor (pre-warmed) vs WITHOUT (empty MCP), N runs each. Reports
...
run(){ # label, withLattice sensor(0/1)
  local label="$1" wcg="$2"
  for i in $(seq 1 "$RUNS"); do
    ...
    if [ "$wcg" = "1" ]; then
      printf '{"mcpServers":{"lattice-sensor":{"command":"env","args":["LATTICE_SENSOR_WASM_RELAUNCHED=1","node","%s","serve","--mcp","--path","%s"]}}}' "$BIN" "$tgt" > "$cfg"
      prewarm "$tgt"
...
echo "== WITH lattice-sensor =="; run with 1
