#!/usr/bin/env bash
# Offload reasoning-OUTPUT-STYLE A/B — all lattice-sensor-on, isolating the Worker's
# output shape's effect on main-session tokens / latency / accuracy:
#   raw  : LATTICE_SENSOR_OFFLOAD_DISABLE=1            (verbatim explore source, the floor)
...
mkcfg() { # file extra-env-pairs(JSON array entries, comma-led or empty)
  printf '{"mcpServers":{"lattice-sensor":{"command":"env","args":["LATTICE_SENSOR_WASM_RELAUNCHED=1"%s,"node","%s","serve","--mcp","--path","%s"]}}}' "$1" "$BIN" "$TARGET"
}
CFG_RAW="$RUNS/mcp-sty-raw-$REPO.json";   mkcfg ',"LATTICE_SENSOR_OFFLOAD_DISABLE=1"' > "$CFG_RAW"
CFG_REFS="$RUNS/mcp-sty-refs-$REPO.json"; mkcfg ",\"LATTICE_SENSOR_OFFLOAD_USAGE_LOG=$USAGE\"" > "$CFG_REFS"
CFG_MAP="$RUNS/mcp-sty-map-$REPO.json";   mkcfg ",\"LATTICE_SENSOR_OFFLOAD_USAGE_LOG=$USAGE\",\"LATTICE_SENSOR_OFFLOAD_STYLE=map\"" > "$CFG_MAP"
CFG_SRC="$RUNS/mcp-sty-src-$REPO.json";   mkcfg ",\"LATTICE_SENSOR_OFFLOAD_USAGE_LOG=$USAGE\",\"LATTICE_SENSOR_OFFLOAD_STYLE=src\"" > "$CFG_SRC"
...
echo "== ARM refs =="; prewarm "$TARGET" "LATTICE_SENSOR_OFFLOAD_USAGE_LOG=$USAGE"
for r in $(seq "$START" "$END"); do run refs "$r" "$CFG_REFS" "$USAGE"; done; kill_daemon
echo "== ARM map ==";  prewarm "$TARGET" "LATTICE_SENSOR_OFFLOAD_USAGE_LOG=$USAGE LATTICE_SENSOR_OFFLOAD_STYLE=map"
for r in $(seq "$START" "$END"); do run map  "$r" "$CFG_MAP"  "$USAGE"; done; kill_daemon
echo "== ARM src ==";  prewarm "$TARGET" "LATTICE_SENSOR_OFFLOAD_USAGE_LOG=$USAGE LATTICE_SENSOR_OFFLOAD_STYLE=src"
