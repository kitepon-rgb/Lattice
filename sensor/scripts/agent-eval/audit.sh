#!/usr/bin/env bash
# One-shot Lattice sensor quality audit:
...
# Env: CORPUS  corpus dir (default: /tmp/lattice-sensor-corpus)
set -uo pipefail
...
# 1. Set the lattice-sensor version under test (mutates the global install).
...
ACTUAL="$(lattice-sensor --version 2>/dev/null || echo '?')"
echo "  lattice-sensor on PATH: $(command -v lattice-sensor) -> $ACTUAL"
...
if ( cd "$REPO_ROOT" && ./scripts/local-install.sh >/dev/null 2>&1 ); then
  echo "  global lattice-sensor restored to dev build"
else
