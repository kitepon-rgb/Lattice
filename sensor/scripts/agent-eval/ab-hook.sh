#!/usr/bin/env bash
# A/B the PreToolUse(Read) REDIRECT hook (P1): does steering Read → lattice_sensor_node
# file-view actually move the agent off Read during implementation? BOTH arms use
# the CURRENT build with lattice-sensor attached and pre-warmed; the only difference is
...
#
#   arm [nohook] — lattice-sensor on, no hook   (does the better file-view get picked on its own?)
#   arm [hook]   — lattice-sensor on, + redirect hook   (does routing close it?)
#
# Reliable attach (works nested): each arm pre-warms a persistent daemon and skips
# the startup re-exec (LATTICE_SENSOR_WASM_RELAUNCHED=1), so claude connects before the
# agent's first turn. Judge by ACTUAL lattice-sensor usage in parse-run.mjs's "by type",
...
echo "###### mcp__lattice_sensor__lattice_sensor_node should rise in the [hook] arm. Logs: $OUT"
