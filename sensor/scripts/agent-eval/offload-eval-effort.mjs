#!/usr/bin/env node
// Effort A/B — does LATTICE_SENSOR_OFFLOAD_EFFORT=high improve offload SYNTHESIS FIDELITY vs low?
// Probe-based (no agent): for each repo × effort × rep, run lattice_sensor_explore with the offload
...
// from agent/adoption noise. Requires `lattice-sensor login` (managed offload) + indexed repos.
//
...
if (typeof Lattice sensor?.openSync !== 'function' || typeof ToolHandler !== 'function') {
...
for (const repo of Object.keys(GT)) {
  ...
  for (const effort of EFFORTS) {
    for (let rep = 1; rep <= REPS; rep++) {
      process.env.LATTICE_SENSOR_OFFLOAD_EFFORT = effort;
      const usageLog = join(tmpdir(), `effort-${repo}-${effort}-${rep}.jsonl`);
      try { rmSync(usageLog); } catch { /* none */ }
      process.env.LATTICE_SENSOR_OFFLOAD_USAGE_LOG = usageLog;
      let answer = '';
      try { answer = (await h.execute('lattice_sensor_explore', { query: GT[repo].question }))?.content?.[0]?.text ?? ''; }
      catch (e) { console.error(`  ${repo}/${effort}#${rep} explore failed: ${e?.message}`); }
      const fired = /Synthesized by Lattice sensor/.test(answer);
