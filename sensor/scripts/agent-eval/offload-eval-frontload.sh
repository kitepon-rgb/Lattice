#!/usr/bin/env bash
# FRONTLOAD arm (approach 1): lattice-sensor attached (offload-disabled) + the front-load
...
printf '{"mcpServers":{"lattice-sensor":{"command":"env","args":["LATTICE_SENSOR_WASM_RELAUNCHED=1","LATTICE_SENSOR_OFFLOAD_DISABLE=1","node","%s","serve","--mcp","--path","%s"]}}}' "$BIN" "$TARGET" > "$CFG"
