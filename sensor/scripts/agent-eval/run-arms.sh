...
#
# Arms vary (exposed lattice-sensor tools, trace-first steering). Tools are trimmed
# SERVER-SIDE via LATTICE_SENSOR_MCP_TOOLS in the MCP config's `env` block, so an
...
if [ -n "$TOOLS" ]; then
  cat > "$CFG" <<JSON
{"mcpServers":{"lattice-sensor":{"command":"$CG_BIN","args":["serve","--mcp","--path","$REPO"],"env":{"LATTICE_SENSOR_MCP_TOOLS":"$TOOLS"}}}}
JSON
else
  cat > "$CFG" <<JSON
