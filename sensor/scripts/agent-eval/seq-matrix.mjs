...
//   1. Trace adoption — on a flow question, does the with-arm actually call lattice_sensor_trace?
...
const MD = new URL('../../docs/benchmarks/lattice-sensor-ab-matrix.md', import.meta.url).pathname;
...
const cgShort = (n) => n.replace('mcp__lattice_sensor__lattice_sensor_', '').replace('mcp__lattice_sensor__', '');
const tag = (n) => n === 'Read' ? 'R' : n === 'Grep' ? 'G' : n === 'Glob' ? 'Gl'
  : n === 'Bash' ? 'B' : n === 'Task' ? 'Ag' : n === 'ToolSearch' ? 'TS'
  : n.includes('lattice-sensor') ? cgShort(n) : n;
...
function parse(file) {
  ...
  for (const l of lines) {
    ...
    if (ev.type === 'system' && ev.subtype === 'init') initCg = (ev.tools || []).filter(t => /lattice-sensor/.test(t)).length;
  ...
  const cg = calls.filter(c => c.name.includes('lattice-sensor'));
  const perTool = {};
...
console.log('\n=== PER-CELL: with-arm lattice-sensor sequence + payload (sorted by repo size) ===');
...
console.log('\n=== lattice-sensor tool usage across all cells (n calls, avg payload/call) ===');
...
console.log(`total lattice-sensor calls=${wCalls} (avg ${(wCalls / cells.length).toFixed(1)}/cell)`);
