...
for (const line of lines) {
  ...
  if (ev.type === 'user' && Array.isArray(ev.message?.content)) {
    for (const b of ev.message.content) {
      ...
      // "Synthesized by Lattice sensor" footer) or a 'refs' answer (carries the re-expanded
      ...
      if (/Synthesized by Lattice sensor|### Referenced source — verbatim/.test(text)) { offloadAnswers.push(text); exploreResults++; }
...
// offload usage sidecar (Lattice sensor AI tokens + cost) — one JSON line per offload call
...
const explore = get('mcp__lattice_sensor__lattice_sensor_explore');
const cgAny = Object.keys(toolCounts).filter(k => /mcp__lattice_sensor__/.test(k)).reduce((s, k) => s + toolCounts[k], 0);
