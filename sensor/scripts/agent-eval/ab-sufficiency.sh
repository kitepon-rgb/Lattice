...
# lattice-sensor (explore/node), does it still Read? Premise under test: explore/node
# return source WITH line numbers, so a Read should not be needed.
#
# WITH lattice-sensor (pre-warmed daemon, reliable nested attach) vs WITHOUT (empty
...
# (configs, docs, a file lattice-sensor didn't index).
#
...
printf '{"mcpServers":{"lattice-sensor":{"command":"env","args":["LATTICE_SENSOR_WASM_RELAUNCHED=1","node","%s","serve","--mcp","--path","%s"]}}}' "$BIN" "$TGT" > "$OUT/mcp-cg.json"

prewarm(){
  pkill -9 -f "serve --mcp --path $TGT" 2>/dev/null
  LATTICE_SENSOR_DAEMON_IDLE_TIMEOUT_MS=1800000 node "$BIN" serve --mcp --path "$TGT" </dev/null >/dev/null 2>&1 &
  node -e 'const fs=require("fs");let n=0;const t=setInterval(()=>{if(fs.existsSync(process.argv[1]+"/.lattice/sensor/daemon.sock")){clearInterval(t);process.exit(0)}if(n++>150){clearInterval(t);process.exit(1)}},100)' "$TGT" >/dev/null 2>&1
}
  ...
  node -e '
    ...
    for(const l of L){try{const o=JSON.parse(l);
      ...
      for(const b of (o.message?.content||[])){if(b.type!=="tool_use")continue;
        if(b.name==="mcp__lattice_sensor__lattice_sensor_explore")ex++;
        else if(b.name==="mcp__lattice_sensor__lattice_sensor_node"){if(b.input&&b.input.symbol)ns++;else nf++;}
        else if(/mcp__lattice_sensor__/.test(b.name))oc++;
...
echo "== WITH lattice-sensor (premise: explore/node used -> Read ~0) =="; run with "$OUT/mcp-cg.json" 1
