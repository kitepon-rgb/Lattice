 ...
 * synchronous SQLite connection serializes every concurrent `lattice_sensor_explore`
 ...
 *     a momentary overload can't teach the agent to abandon lattice sensor — instead
...
/**
 * Resolve the pool size from the `LATTICE_SENSOR_QUERY_POOL_SIZE` override and the
...
function resolveBusyTimeoutMs(): number {
  const raw = process.env.LATTICE_SENSOR_QUERY_BUSY_TIMEOUT_MS;
...
function busyGuidance(waitedMs: number): ToolResult {
  ...
  return {
    content: [{
      type: 'text',
      text:
        `LatticeSensor is busy serving other concurrent requests right now (this call waited ${secs}s in the queue). ` +
...
export class QueryPool {
  ...
  private onWorkerGone(w: PoolWorker): void {
    ...
    if (job) {
      if (job.retries < this.maxRetries && this.healthy) {
        ...
        this.settle(job, { isError: true, content: [{ type: 'text', text: 'lattice sensor worker crashed; please retry the call.' }] });
      }
    }
    this.drain();
  }
  ...
  async destroy(): Promise<void> {
    ...
    for (const job of [...this.inflight.values(), ...this.queue]) {
      this.settle(job, { isError: true, content: [{ type: 'text', text: 'lattice sensor is shutting down; retry shortly.' }] });
    }
    this.inflight.clear();
    this.queue = [];
