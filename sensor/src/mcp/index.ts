...
import { DATABASE_FILENAME } from '../db/index';
 ...
 * appended to `.lattice/sensor/daemon.log`. Before #1431 no log line carried a
...
export function watchdogProgressPaths(root: string | null): { progressPaths?: string[] } {
  if (!root) return {};
  const dbPath = path.join(getLatticeSensorDir(root), DATABASE_FILENAME);
...
export class MCPServer {
  ...
  private async startDaemonProcess(): Promise<void> {
    // In daemon mode stderr IS `.lattice/sensor/daemon.log`; stamp every line so
