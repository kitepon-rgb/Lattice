 ...
 * `node:sqlite`. `lattice_sensor_explore` is CPU-heavy (FTS + RWR/personalized-
 ...
 * worktree), `lattice_sensor_status`, and telemetry, none of which a watcher-less
...
// Mirror the engine's lazy-require of the heavy LatticeSensor + tools chain. This
...
if (parentPort) {
  ...
  try {
    const cg = loadLatticeSensor().openSync(root);
  ...
  const serve = async (msg: CallMessage): Promise<void> => {
    ...
    if (msg.toolName === '__test_crash__' && process.env.LATTICE_SENSOR_QUERY_WORKER_ALLOW_TEST_CRASH === '1') {
      process.exit(13);
    }
    if (!handler) {
      port.postMessage({
        type: 'result',
        id: msg.id,
        result: errorResult(`lattice sensor worker could not open the project: ${initError}`),
      });
      return;
    }
    try {
