...
export function formatDegradedBanner(reason: string | null): string {
  return (
    '⚠️ LatticeSensor auto-sync is DISABLED — live file watching stopped, so the index is ' +
...
export function getStaticTools(): ToolDefinition[] {
  ...
  if (!raw || !raw.trim()) {
    return tools.filter(t => DEFAULT_MCP_TOOLS.has(t.name.replace(/^lattice_sensor_/, '')));
  }
  const allow = new Set(raw.split(',').map(s => s.trim().replace(/^lattice_sensor_/, '')).filter(Boolean));
  return allow.size ? tools.filter(t => allow.has(t.name.replace(/^lattice_sensor_/, ''))) : tools;
}
...
export class ToolHandler {
  ...
  private projectCache: Map<string, LatticeSensor> = new Map();
  ...
  constructor(private cg: LatticeSensor | null) {}
    ...
    try {
      ...
      if (outcome === 'timeout') {
        process.stderr.write(
          `[LatticeSensor MCP] Catch-up reconcile still running after ${timeoutMs}ms; serving this tool call now and finishing the reconcile in the background (#905). ` +
  ...
  private toolAllowlist(): Set<string> | null {
    ...
    const short = (s: string) => s.trim().replace(/^lattice_sensor_/, '');
  ...
  getTools(): ToolDefinition[] {
    ...
    let visible = allow
      ? tools.filter(t => allow.has(t.name.replace(/^lattice_sensor_/, '')))
      : tools.filter(t => DEFAULT_MCP_TOOLS.has(t.name.replace(/^lattice_sensor_/, '')));
   ...
   */
  private getLatticeSensor(projectPath?: string): LatticeSensor {
    if (!projectPath) {
      if (!this.cg) {
        ...
        if (this.openFailure) {
          throw new IndexOpenError(
            `The default project's LatticeSensor index failed to open: ${this.openFailure.message}\n` +
        ...
        throw new NotIndexedError(
          'No LatticeSensor project is loaded for this session.\n' +
    ...
    const resolvedRoot = findNearestLatticeSensorRoot(projectPath);
   ...
   * tool call.
   */
  private freshen(cg: LatticeSensor): LatticeSensor {
    try {
      if (cg.reopenIfReplaced()) {
        process.stderr.write(
          '[LatticeSensor MCP] The index was replaced on disk (e.g. a git worktree ' +
  ...
  private async handleSearch(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    const cg = this.getLatticeSensor(args.projectPath as string | undefined);
   ...
   */
  private buildFlowFromNamedSymbols(cg: LatticeSensor, query: string): { text: string; pathNodeIds: Set<string>; namedNodeIds: Set<string>; uniqueNamedNodeIds: Set<string>; spineCallSites: Map<string, number> } {
   ...
   */
  private buildDynamicBoundaries(cg: LatticeSensor, scanList: Node[], named: Map<string, Node>): string {
   ...
   */
  private buildPolymorphicBoundaries(cg: LatticeSensor, candidates: Array<{ token: string; family: Node[] }>, named: Map<string, Node>): string {
   ...
   */
  private boundaryCandidates(cg: LatticeSensor, key: string, keyIsType: boolean, named: Map<string, Node>, selfId: string): string {
   ...
   */
  private buildBlastRadiusSection(cg: LatticeSensor, subgraph: Subgraph): string {
  ...
  private async handleNode(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    if (listed.length) {
      ...
      out.push(
        '',
        `> Need one of these in full? Call lattice_sensor_node again with \`file\` (e.g. \`"${listed[0]!.filePath.split('/').pop()}"\`) or \`line\` — do NOT Read it.`,
      );
    }
  ...
  private async handleFileView(
    ...
    if (allFiles.length === 0) return this.textResult('No files indexed. Run `lattice sensor index` first.');
    ...
    if (!resolved) {
      return this.textResult(
        `No indexed file matches "${fileArg}". Lattice sensor indexes source files; configs/docs it doesn't parse won't appear — Read those directly.`,
      );
    }
    ...
    if (!complete) {
      out.push(
        '',
        `(lines ${offset}–${shownEnd} of ${total} — pass \`offset\`/\`limit\` for another range, or \`lattice_sensor_node <symbol>\` for one symbol in full)`,
      );
    }
   ...
   */
  private formatTrail(cg: LatticeSensor, node: Node): string {
    const TRAIL_CAP = 12;
    ...
    const lines: string[] = ['', '**Trail — lattice_sensor_node any of these to follow it (no Read needed)**'];
    if (callees.length > 0) {
  ...
  private async handleStatus(args: Record<string, unknown>): Promise<ToolResult> {
    let cg = this.getLatticeSensor(args.projectPath as string | undefined);
    ...
    lines.push(
      ...
      `**Lattice sensor version:** ${LatticeSensorPackageVersion}`,
   ...
   */
  private findSymbolMatches(cg: LatticeSensor, symbol: string): Node[] {
   ...
   */
  private findAllSymbols(cg: LatticeSensor, symbol: string): { nodes: Node[]; note: string } {
   ...
   */
  private buildContainerOutline(cg: LatticeSensor, node: Node): string {
  ...
  private formatNodeDetails(node: Node, code: string | null, outline?: string | null): string {
    ...
    if (outline) {
      lines.push('', outline, '',
        `> Structural outline only. Read \`${node.filePath}\` or call lattice_sensor_node on a specific member for its body.`);
    } else if (code) {
