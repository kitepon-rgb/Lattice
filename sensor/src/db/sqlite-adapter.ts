 ...
 * is storage-agnostic.
 *
 * LatticeSensor ships with a bundled Node runtime, so `node:sqlite` (real SQLite,
 ...
 * so `lattice sensor status` and the per-instance reporting have a stable shape.
 */
...
export function createDatabase(dbPath: string, opts?: { readOnly?: boolean }): { db: SqliteDatabase; backend: SqliteBackend } {
  try {
    ...
    throw new Error(
      ...
      'LatticeSensor requires node:sqlite (Node.js 22.5+). Install the self-contained\n' +
      'LatticeSensor release (it bundles a compatible Node), or run on Node 22.5+.\n' +
