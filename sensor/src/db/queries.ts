...
export class QueryBuilder {
  ...
  insertNode(node: Node): void {
    ...
    if (!node.id || !node.kind || !node.name || !node.filePath || !node.language) {
      console.error('[LatticeSensor] Skipping node with missing required fields:', {
        id: node.id,
        kind: node.kind,
        name: node.name,
  ...
  updateNode(node: Node): void {
    ...
    if (!node.id || !node.kind || !node.name || !node.filePath || !node.language) {
      console.error('[LatticeSensor] Skipping node update with missing required fields:', node.id);
      return;
    }
   ...
   * "Glob + Read routes.rb" pattern that beats lattice sensor on tiny realworld
   * template repos.
   *
