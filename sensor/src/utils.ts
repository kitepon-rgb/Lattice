 ...
 * @example
 * ```typescript
 * import { Mutex, processInBatches, MemoryMonitor, validatePathWithinRoot } from 'lattice sensor';
 *
 ...
 * API key, JDBC URL with embedded creds). LatticeSensor must surface the KEY only
 ...
 * (lattice_sensor_node `includeCode`, lattice_sensor_explore source) go through here, so
  ...
  acquire(): void {
    ...
    if (fs.existsSync(this.lockPath)) {
      try {
        ...
        if (lockAge < FileLock.STALE_TIMEOUT_MS && !isNaN(pid) && this.isProcessAlive(pid)) {
          throw new Error(
            `LatticeSensor database is locked by another process (PID ${pid}). ` +
            `If this is stale, run 'lattice sensor unlock' or delete ${this.lockPath}`
          );
        }
    ...
    try {
      ...
      if (err.code === 'EEXIST') {
        ...
        throw new Error(
          'LatticeSensor database is locked by another process. ' +
          `If this is stale, run 'lattice sensor unlock' or delete ${this.lockPath}`
        );
      }
      throw err;
