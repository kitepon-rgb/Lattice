 ...
 * `lattice sensor index` uses every core instead of pinning one.
 *
 ...
 *     conservative rollback: set `LATTICE_SENSOR_PARSE_WORKERS=1`.
 *
 ...
 *  the project's lattice-sensor.json extension overrides) and handed to the worker. */
...
/**
 * Resolve the pool size from the `LATTICE_SENSOR_PARSE_WORKERS` override and the
 ...
 */
/**
 * Resolve the base per-parse timeout from the `LATTICE_SENSOR_PARSE_TIMEOUT_MS`
