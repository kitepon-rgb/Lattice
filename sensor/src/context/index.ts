...
export class ContextBuilder {
  ...
  private buildLowConfidenceNote(entryPoints: Node[]): string {
    ...
    const dirLine = dirs.length
      ? `\n- \`lattice_sensor_files\` a likely area: ${dirs.map(d => `\`${d}\``).join(', ')}`
      : '';
    return `\n\n${LOW_CONFIDENCE_MARKER}\n\n`
      ...
      + '- `lattice_sensor_explore` with the **exact symbol names** you are after '
      + '(class / function / method names), or\n'
      + '- `lattice_sensor_search <name>` for one specific symbol'
      + dirLine
   ...
   * truncation is honest, and the agent can lattice_sensor_node the last hop to bridge.
   */
  private buildCallPathsSection(subgraph: Subgraph): string {
    ...
    const lines = [
      ...
      '',
      hasSynth
        ? '_Hops marked `[callback/event …]` are dynamic dispatch bridged by lattice sensor (with the registration site); the rest are direct calls. lattice_sensor_node any symbol for its body._'
        : '_lattice_sensor_node any symbol above for its source + its own callers/callees._',
    ];
    return '\n' + lines.join('\n') + '\n';
  }
