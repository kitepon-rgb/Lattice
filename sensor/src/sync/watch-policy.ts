 ...
 *  1. `LATTICE_SENSOR_NO_WATCH=1`    → off  (explicit opt-out always wins)
 *  2. `LATTICE_SENSOR_FORCE_WATCH=1` → on   (overrides auto-detection)
...
export function watchDisabledReason(projectRoot: string, probe: WatchProbe = {}): string | null {
  ...
  if (env.LATTICE_SENSOR_NO_WATCH === '1') {
    return 'LATTICE_SENSOR_NO_WATCH=1 is set';
  }
  if (env.LATTICE_SENSOR_FORCE_WATCH === '1') {
    return null;
  }
