...
export const TELEMETRY_ENDPOINT = 'https://telemetry.invalid/v1/events';
export const TELEMETRY_DOCS = 'https://github.com/kitepon-rgb/Lattice/blob/main/TELEMETRY.md';
...
export interface TelemetryStatus {
  ...
  decidedBy: 'DO_NOT_TRACK' | 'LATTICE_SENSOR_TELEMETRY' | 'config' | 'default';
...
export class Telemetry {
  ...
  constructor(opts: TelemetryOptions = {}) {
    this.dir = opts.dir ?? path.join(os.homedir(), '.lattice sensor');
   ...
   * DO_NOT_TRACK=1 > LATTICE_SENSOR_TELEMETRY=0|1 > stored config > default on.
   */
  getStatus(): TelemetryStatus {
    ...
    const forced = this.env.LATTICE_SENSOR_TELEMETRY;
    if (forced !== undefined && forced !== '') {
      const on = forced !== '0' && forced.toLowerCase() !== 'false';
      return { enabled: on, decidedBy: 'LATTICE_SENSOR_TELEMETRY', machineId, configPath: this.configPath };
    }
    if (config) {
  ...
  /**
   * Persist an explicit user choice (installer toggle or `lattice sensor
  ...
  private firstRunNotice(): void {
    ...
    this.writeStderr(
      `lattice sensor collects anonymous usage stats (no code, paths, or names) — ` +
      `"lattice sensor telemetry off" or LATTICE_SENSOR_TELEMETRY=0 disables. Details: ${TELEMETRY_DOCS}\n`,
    );
  }
  ...
  persistSync(): void {
    ...
    this.counts.clear();
    this.events = [];
    // Re-check at persist time: `lattice sensor telemetry off` mid-process must not
  ...
  private async send(lines: BufferLine[], timeoutMs: number): Promise<BufferLine[]> {
    ...
    const envelope = {
      ...
      sensor_version: this.packageVersion(),
      os: process.platform,
      arch: process.arch,
    ...
    const endpoint = this.env.LATTICE_SENSOR_TELEMETRY_ENDPOINT || TELEMETRY_ENDPOINT;
  ...
  private debug(msg: string): void {
    if (this.env.LATTICE_SENSOR_TELEMETRY_DEBUG === '1') {
      this.writeStderr(`[lattice sensor telemetry] ${msg}\n`);
    }
  }
}
 ...
 * `telemetry.Lattice.com`) is a product-identity violation for a forked
 ...
 * `LATTICE_SENSOR_TELEMETRY` and any stored config), so forcing it truthy here —
