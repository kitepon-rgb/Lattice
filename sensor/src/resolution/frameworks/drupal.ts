 ...
 *    canonical `hook_X` name, linking implementations to the hook when `lattice_sensor_callers`
 *    is invoked.
 *
 ...
 *   `lattice_sensor_search("form_alter")`. Full hook-node creation (virtual nodes for every hook)
 ...
 * - TODO: Improve hook resolution: create virtual `hook_*` nodes so `lattice_sensor_callers`
