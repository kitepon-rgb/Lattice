 ...
 *
 * There is no `writeClaudeMd` shim anymore: latticeSensor stopped writing a
...
export function hasMcpConfig(location: InstallLocation): boolean {
  ...
  return !!config.mcpServers?.latticeSensor;
}
...
export function hasPermissions(location: InstallLocation): boolean {
  ...
  return allow.some((p: string) => p.startsWith('mcp__lattice_sensor__'));
}
