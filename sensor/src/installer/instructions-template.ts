 ...
 *    the latticeSensor MCP tools only as deferred names and rarely think to
 *    load them: measured on a forced-delegation flow question (excalidraw,
 *    sonnet, high effort), subagents loaded + used latticeSensor in ~1 of 9
 ...
 *    run the `latticeSensor explore` CLI, which prints the same output as the
 *    MCP tool.
 *
...
export const LATTICE_SENSOR_SECTION_START = '<!-- LATTICE_SENSOR_START -->';
export const LATTICE_SENSOR_SECTION_END = '<!-- LATTICE_SENSOR_END -->';
 ...
 * indexed" claim would send subagents into failing latticeSensor calls (the
 * noise the unindexed-session policy exists to prevent).
 */
export const LATTICE_SENSOR_INSTRUCTIONS_BLOCK = `${LATTICE_SENSOR_SECTION_START}
## LatticeSensor

In repositories indexed by LatticeSensor (a \`.lattice/sensor/\` directory exists at the repo root), reach for it BEFORE grep/find or reading files when you need to understand or locate code:
