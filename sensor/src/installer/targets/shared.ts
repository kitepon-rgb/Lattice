...
import {
  LATTICE_SENSOR_INSTRUCTIONS_BLOCK,
  LATTICE_SENSOR_SECTION_START,
  LATTICE_SENSOR_SECTION_END,
} from '../instructions-template';

/**
 * The MCP-server config block latticeSensor injects. Same shape across
 ...
 * `lattice_sensor_explore` is even LISTED to the agent (see DEFAULT_MCP_TOOLS in
 ...
 * LATTICE_SENSOR_MCP_TOOLS, it's already pre-approved (no permission prompt, no
 ...
 * is the way to allow-all for one server; a bare `mcp__lattice_sensor` or `*` is
 * ignored. The allowlist gates PROMPTING, not visibility, so a superset here
 * never makes a hidden tool appear.
 */
export function getLatticeSensorPermissions(): string[] {
  return ['mcp__lattice_sensor__*'];
}
 ...
 *
 * Used by Claude / Codex for the `<!-- LATTICE_SENSOR_START --> ... <!--
 * LATTICE_SENSOR_END -->` block. Preserves all content outside the
 * markers verbatim.
 *
...
/**
 * Upsert the LatticeSensor instructions block into an agent instructions
