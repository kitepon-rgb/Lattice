 ...
 *     `./.kiro/settings/mcp.json` (local). Standard `mcpServers.lattice/sensor`
 *     shape, same as Claude / Cursor / Gemini.
 *   - Instructions to `~/.kiro/steering/latticeSensor.md` (global) or
 *     `./.kiro/steering/latticeSensor.md` (local). Kiro's "steering" system
 *     loads every `*.md` file in the steering dir as agent context, so
 *     a dedicated `latticeSensor.md` is the natural surface — we own the
...
function steeringPath(loc: Location): string {
  return path.join(configDir(loc), 'steering', 'latticeSensor.md');
}
...
class KiroTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    // The steering doc is no longer written — the latticeSensor usage
    // guidance ships in the MCP server's `initialize` response (issue
    // #529). Delete a `latticeSensor.md` a previous install created so an
    // upgrade self-heals.
 ...
 * out of recognition we still remove it — latticeSensor.md is a name we
