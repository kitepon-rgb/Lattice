 ...
 *
 *   mcp_servers.lattice/sensor -> `latticeSensor serve --mcp`
 *   platform_toolsets.cli -> `mcp-latticeSensor`
 *
 * The second entry matters because Hermes CLI profiles often enable an
 * explicit `platform_toolsets.cli` list. Without `mcp-latticeSensor` in that
...
class HermesTarget implements AgentTarget {
  ...
  detect(loc: Location): DetectionResult {
    ...
    return {
      installed,
      alreadyConfigured: hasLatticeSensorMcpServer(content),
      configPath: file,
    };
  }
  ...
  uninstall(loc: Location): WriteResult {
    ...
    const after = removeLatticeSensorToolset(removeLatticeSensorMcpServer(before));
    if (after === before) {
      return { files: [{ path: file, action: 'not-found' }] };
    }
  ...
  printConfig(loc: Location): string {
    ...
    return [
      `# Add to ${configPath()}`,
      '',
      renderLatticeSensorMcpBlock().join('\n'),
      '',
      'platform_toolsets:',
      '  cli:',
      '    - hermes-cli',
...
function writeHermesConfig(): WriteResult['files'][number] {
  ...
  const before = readText(file);
  const afterMcp = upsertLatticeSensorMcpServer(before);
  const after = upsertLatticeSensorToolset(afterMcp);
...
function renderLatticeSensorMcpChild(): string[] {
  return [
    '  latticeSensor:',
...
function renderLatticeSensorMcpBlock(): string[] {
  return ['mcp_servers:', ...renderLatticeSensorMcpChild()];
}

function hasLatticeSensorMcpServer(content: string): boolean {
  const lines = splitLines(content);
  const parent = topLevelRange(lines, 'mcp_servers');
  return !!parent && !!childRange(lines, parent, 'latticeSensor');
}

function upsertLatticeSensorMcpServer(content: string): string {
  const lines = splitLines(content);
  const parent = topLevelRange(lines, 'mcp_servers');
  const child = parent ? childRange(lines, parent, 'latticeSensor') : null;
  const replacement = renderLatticeSensorMcpChild();

  if (!parent) {
    if (lines.length > 0 && lines[lines.length - 1] === '') lines.pop();
    if (lines.length > 0) lines.push('');
    lines.push(...renderLatticeSensorMcpBlock());
...
function removeLatticeSensorMcpServer(content: string): string {
  const lines = splitLines(content);
  const parent = topLevelRange(lines, 'mcp_servers');
  const child = parent ? childRange(lines, parent, 'latticeSensor') : null;
...
function upsertLatticeSensorToolset(content: string): string {
  const lines = splitLines(content);
  ...
  if (!parent) {
    if (lines.length > 0 && lines[lines.length - 1] === '') lines.pop();
    if (lines.length > 0) lines.push('');
    lines.push('platform_toolsets:', '  cli:', '    - hermes-cli', '    - mcp-latticeSensor');
    return joinLines(lines);
  }

  if (!cli) {
    lines.splice(parent.end, 0, '  cli:', '    - hermes-cli', '    - mcp-latticeSensor');
    return joinLines(lines);
  }

  const hasEntry = lines
    .slice(cli.start + 1, cli.end)
    .some((line) => line.trim() === '- mcp-latticeSensor');
  if (hasEntry) return joinLines(lines);

  lines.splice(cli.end, 0, `${cli.itemIndent}- mcp-latticeSensor`);
  return joinLines(lines);
}

function removeLatticeSensorToolset(content: string): string {
  const lines = splitLines(content);
  const parent = topLevelRange(lines, 'platform_toolsets');
  const cli = parent ? listChildBlock(lines, parent, 'cli') : null;
  ...
  const next = lines.filter((line, idx) => {
    ...
    return line.trim() !== '- mcp-latticeSensor';
  });
