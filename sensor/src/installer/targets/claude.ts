 ...
 * latticeSensor entry out of it on install and strip it on uninstall.
...
class ClaudeCodeTarget implements AgentTarget {
  ...
  detect(loc: Location): DetectionResult {
    ...
    const alreadyConfigured = !!config.mcpServers?.latticeSensor;
  ...
  install(loc: Location, opts: InstallOptions): WriteResult {
    ...
    // versions wrote `latticeSensor mark-dirty` / `sync-if-dirty` hooks to
    ...
    // 3. CLAUDE.md instructions — the short marker-fenced LatticeSensor
    // block (#704). The MCP initialize instructions reach only the main
    // agent; CLAUDE.md is what Task-tool subagents (and non-MCP
    // harnesses) actually see, so the block carries the latticeSensor
  ...
  uninstall(loc: Location): WriteResult {
    ...
    const mcpPath = mcpJsonPath(loc);
    const config = readJsonFile(mcpPath);
    if (config.mcpServers?.latticeSensor) {
      delete config.mcpServers.latticeSensor;
    ...
    // 1b. Also strip the latticeSensor entry from a legacy ./.claude.json
    ...
    if (Array.isArray(settings.permissions?.allow)) {
      ...
      settings.permissions.allow = settings.permissions.allow.filter(
        (p: string) => !p.startsWith('mcp__lattice_sensor__'),
      );
    ...
    // 3. Instructions — strip the legacy LatticeSensor block if present.
  ...
  printConfig(loc: Location): string {
    ...
    const snippet = JSON.stringify({ mcpServers: { latticeSensor: getMcpServerConfig() } }, null, 2);
...
export function writeMcpEntry(loc: Location): WriteResult['files'][number] {
  ...
  const before = existing.mcpServers?.latticeSensor;
  ...
  // `latticeSensor` key) is 'updated', not 'created' — we're adding an
  ...
  existing.mcpServers.latticeSensor = after;
  writeJsonFile(file, existing);
  return { path: file, action };
}

/**
 * Strip the latticeSensor entry from a legacy project-local
 * `./.claude.json` (written by pre-#207 installers, which Claude Code
 * never read). Surgical: only our `latticeSensor` key is removed; sibling
...
function cleanupLegacyLocalMcp(): WriteResult['files'][number] | null {
  ...
  if (!config.mcpServers?.latticeSensor) return null;
  delete config.mcpServers.latticeSensor;
 ...
 */
function isLegacyLattice sensorHookCommand(command: unknown): boolean {
  if (typeof command !== 'string') return false;
  return (
    command.includes('latticeSensor mark-dirty') ||
    command.includes('latticeSensor sync-if-dirty')
  );
}
...
/**
 * Remove stale latticeSensor auto-sync hooks from Claude `settings.json`.
 *
 * Surgical at the individual-command level: only entries matching
 * `isLegacyLattice sensorHookCommand` are dropped, so a sibling hook sharing
 ...
 * latticeSensor command, so a settings.json with no legacy hooks is left
...
/**
 * Remove stale latticeSensor auto-sync hooks (`mark-dirty` / `sync-if-dirty`) that a
...
export function cleanupLegacyHooks(loc: Location): WriteResult['files'][number] {
  return removeHookCommandsMatching(loc, isLegacyLattice sensorHookCommand);
}
...
export function writePermissionsEntry(loc: Location): WriteResult['files'][number] {
  ...
  const want = getLatticeSensorPermissions();
 ...
 * a `command` hook that runs `latticeSensor prompt-hook`, which injects
 * lattice_sensor_explore context for structural prompts so the agent reliably uses
...
export function writePromptHookEntry(loc: Location): WriteResult['files'][number] {
  ...
  // `lattice-sensor prompt-hook`, which Git Bash resolves to nothing; a
...
/**
 * Strip the marker-delimited LatticeSensor block from CLAUDE.md if a prior
 * install wrote one. Lattice sensor no longer maintains an instructions file
