 ...
 * was then auto-migrated by Antigravity, and re-ran `latticeSensor install`
 * doesn't end up with stale latticeSensor entries in two files.
 *
 ...
 * to get latticeSensor to appear in the Customizations UI. We build the
 ...
 * tools live outside that, so a bare `latticeSensor` command fails to spawn
 * even when `which latticeSensor` resolves in the user's shell. We resolve
 * `latticeSensor` to its absolute path on macOS at install time. (Linux GUI
 ...
 * still get a working MCP integration; the prefer-latticeSensor-over-grep
...
/**
 * Resolve the on-disk path of the `latticeSensor` binary so a Mac GUI app
 * launched from Dock/Finder (with a stripped PATH) can find it. Falls
 * back to the bare `latticeSensor` name when:
 *
 ...
 */
function resolveLattice sensorCommand(): string {
  if (process.platform !== 'darwin') return 'latticeSensor';
  try {
    const resolved = execSync('command -v latticeSensor || which latticeSensor', {
      encoding: 'utf-8',
  ...
  return 'latticeSensor';
}

/**
 * Build the latticeSensor MCP-server entry for Antigravity. Distinct from
...
function buildAntigravityEntry(): { command: string; args: string[] } {
  return {
    command: resolveLattice sensorCommand(),
    args: ['serve', '--mcp'],
  };
}
...
class AntigravityTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    // wind up with two competing latticeSensor configs.
  ...
  uninstall(loc: Location): WriteResult {
    ...
    files.push(removeLattice sensorFromFile(preferred));

    // Also sweep the OTHER path (legacy when preferred is unified, and
    // vice versa) — handles the migration-half-state case where latticeSensor
    ...
    if (preferred !== other) {
      const otherResult = removeLattice sensorFromFile(other);
  ...
  printConfig(loc: Location): string {
    ...
    const file = preferredMcpConfigPath();
    const snippet = JSON.stringify({ mcpServers: { latticeSensor: buildAntigravityEntry() } }, null, 2);
    return `# Add to ${file}\n\n${snippet}\n`;
  }
...
/**
 * Strip the latticeSensor entry from the legacy `~/.gemini/antigravity/mcp_config.json`
 * if it's present AND we're writing to the unified path. Used by install
 * to migrate users who had latticeSensor configured on the legacy path
...
function removeLattice sensorFromFile(file: string): WriteResult['files'][number] {
  if (!fs.existsSync(file)) return { path: file, action: 'not-found' };
  const config = readJsonFile(file);
  if (!config.mcpServers?.latticeSensor) return { path: file, action: 'not-found' };
  delete config.mcpServers.latticeSensor;
  if (Object.keys(config.mcpServers).length === 0) {
    delete config.mcpServers;
  }
