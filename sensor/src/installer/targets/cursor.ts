 ...
 *     as Claude.
 *   - Instructions to `./.cursor/rules/latticeSensor.mdc` (project-local
 ...
 * `workspaceFolders` in the MCP initialize call. The latticeSensor MCP
 * server's `process.cwd()` fallback therefore misses the workspace's
 * `.lattice/sensor/` and reports "not initialized" on every tool call.
 *
...
function rulesPath(): string {
  return path.join(process.cwd(), '.cursor', 'rules', 'latticeSensor.mdc');
}
...
class CursorTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    // We no longer write `.cursor/rules/latticeSensor.mdc` — the latticeSensor
  ...
  printConfig(loc: Location): string {
    ...
    const snippet = JSON.stringify({ mcpServers: { latticeSensor: buildCursorMcpConfig(loc) } }, null, 2);
...
/**
 * Build the latticeSensor MCP-server config for Cursor at the given
 ...
 *
 * Unlike the shared CLAUDE.md / AGENTS.md files (where latticeSensor owns
 * only a marker-delimited section), `.cursor/rules/latticeSensor.mdc` is a
 ...
 * block but leave the orphaned `description: LatticeSensor ...` frontmatter
 * behind, so the file lingers and still "mentions" latticeSensor.
 *
...
function removeRulesEntry(): WriteResult['files'][number] {
  ...
  const startIdx = content.indexOf(LATTICE_SENSOR_SECTION_START);
  const endIdx = content.indexOf(LATTICE_SENSOR_SECTION_END);

  // Our marked block is present — strip it, then decide what's left.
  if (startIdx !== -1 && endIdx > startIdx) {
    const before = content.substring(0, startIdx).trimEnd();
    const after = content.substring(endIdx + LATTICE_SENSOR_SECTION_END.length).trimStart();
