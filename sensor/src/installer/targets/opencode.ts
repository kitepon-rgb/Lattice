 ...
 *     latticeSensor entry out of the legacy `%APPDATA%/opencode` location.
...
function cleanupLegacyWindowsState(): WriteResult['files'] {
  ...
  const action = removeMarkedSection(agents, LATTICE_SENSOR_SECTION_START, LATTICE_SENSOR_SECTION_END);
  if (action === 'removed') out.push({ path: agents, action });
  return out;
}

/**
 * Strip the marker-delimited LatticeSensor block from AGENTS.md if a prior
