...
impl<'t> Walker<'t> {
    ...
    fn extract_call(&mut self, node: Node<'t>) {
        ...
        // `require`/`require_relative` nested inside a method body (Lattice
        // efc8299; ADR 0154). extract_import emits the same node/ref pair the
        // top-level path does — module-name ref AND the resolved file-path
        // ref — attributed to the enclosing scope (top_row == callerId).
