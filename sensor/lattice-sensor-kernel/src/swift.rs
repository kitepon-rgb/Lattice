...
impl<'t> Walker<'t> {
    ...
    fn extract_struct(&mut self, node: Node<'t>) {
        ...
        self.extract_inheritance(node, row);
        // Lattice's wasm arm runs extractDecoratorsFor on the struct path too
        // (`@main struct` emits decorates main) — the upstream port's "structs
        // emit nothing" note described upstream's older TS, not ours.
        self.extract_decorators_for(node, row);
    ...
    fn extract_decorators_for(&mut self, decl: Node<'t>, decorated_row: u32) {
        // Decorator-inclusive first line (schema v11) — Lattice extension; see ADR 0154.
        let mut extent: Option<u32> = None;
        for i in 0..decl.named_child_count() {
            let Some(child) = decl.named_child(i) else { continue };
            if let Some(r) = self.consider_decorator(child, decorated_row) {
                    if extent.is_none_or(|c| r < c) { extent = Some(r); }
                }
            if child.kind() == "modifiers" {
                for j in 0..child.named_child_count() {
                    if let Some(m) = child.named_child(j) {
                        if let Some(r) = self.consider_decorator(m, decorated_row) {
                    if extent.is_none_or(|c| r < c) { extent = Some(r); }
                }
                    }
                }
        ...
        if decl_idx > 0 {
            ...
            while j >= 0 {
                ...
                if !matches!(sib.kind(), "decorator" | "annotation" | "marker_annotation" | "attribute" | "attribute_item") {
                    break;
                }
                if let Some(r) = self.consider_decorator(sib, decorated_row) {
                    if extent.is_none_or(|c| r < c) { extent = Some(r); }
                }
                j -= 1;
            }
    ...
    fn flush_value_refs(&mut self, root: Node<'t>) {
        ...
        if std::env::var("LATTICE_SENSOR_VALUE_REFS").as_deref() == Ok("0") {
            return;
        }
        ...
        while let Some(n) = dstack.pop() {
            ...
            dvisited += 1;
            // Assignment-derived pruning removed (Lattice; ADR 0154). The wasm
            // arm's write-detection rework stopped counting `X = …` toward the
            // shadow prune — a declared-then-assigned `let X: Int` stays a
            // value-ref target, and swift's `directly_assignable_expression`
            // wrapper never matched the wasm scan's bare-identifier case
            // anyway. Only declaration shadowing (property_declaration) prunes.
