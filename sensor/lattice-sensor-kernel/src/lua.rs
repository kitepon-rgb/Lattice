...
impl<'t> Walker<'t> {
    ...
    fn extract_call(&mut self, node: Node<'t>) {
        ...
        // Lua/Luau `require(...)` nested inside a function body (Lattice
        // efc8299; ADR 0154). Top-level requires are claimed by the
        // visit_node ladder before extract_call ever sees them, so this
        // branch only fires for the nested case — no double emit.
        if let Some(module) = self.require_module(node) {
            self.emit_require(node, &module);
            return;
        }
