...
impl<'t> Walker<'t> {
    ...
    fn extract_fn_or_method(&mut self, node: Node<'t>) {
        ...
        // extractDecoratorsFor: rust attribute_items are siblings — the
        // decorates ref stays a no-op, but the extent widening applies.
        self.apply_attribute_extent(node, row);
    ...
    /// Decorator-inclusive first line (schema v11) — Lattice extension.
    /// Rust attributes are PRECEDING SIBLINGS (`#[derive(...)]` above the
    /// item). extractDecoratorsFor's sibling scan treats attribute_item as
    /// extent-only: no `decorates` ref, but the lines must be covered —
    /// cutting a struct away from its derive silently changes behavior.
    fn apply_attribute_extent(&mut self, decl: Node<'t>, row: u32) {
        ...
        let Some(di) = decl_idx else { return };
        ...
        for j in (0..di).rev() {
            ...
            let r = sib.start_position().row as u32 + 1;
            if extent.is_none_or(|c| r < c) {
                extent = Some(r);
            }
        }
        if let Some(r) = extent {
            if r < self.tables.node_start_line(row) {
                let json = format!("{{\"extentStartLine\":{r}}}");
                let slot = self.arena.put(&json);
                self.tables.set_node_extra_json(row, slot);
            }
        }
    }
