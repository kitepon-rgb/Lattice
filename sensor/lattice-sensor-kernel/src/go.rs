...
fn is_value_write_ref(mut node: Node) -> bool {
    ...
    while let Some(owner) = parent {
        if matches!(owner.kind(), "selector_expression" | "index_expression" | "slice_expression") {
            let Some(root) = owner.child_by_field_name("operand") else { break };
    ...
    if owner.kind() == "assignment_statement" {
        return owner.child_by_field_name("left").is_some_and(|left| same_span(left, node));
    }
    matches!(owner.kind(), "inc_statement" | "dec_statement")
}
