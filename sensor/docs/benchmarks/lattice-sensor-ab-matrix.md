# Lattice Sensor A/B benchmark — with vs without, every language × S/M/L

**Date:** 2026-05-24 · **Branch:** `main` · **lattice-sensor 0.9.4**

A headless agent (Claude Opus, `--permission-mode bypassPermissions`) answers one
**canonical flow question** per repo — twice: **with** the lattice-sensor MCP server, and
**without** any MCP (built-in Read/Grep/Glob/Bash only). Same model, same prompt; lattice-sensor
...
**Across 37 cells, lattice-sensor cut total file reads from 159 → 38 — 76% fewer.** It never
*increased* reads in any cell (0 regressions). The mechanism: a few sub-millisecond lattice-sensor
...
resolves in <10 calls and never balloons, so it doesn't reach the regime where lattice-sensor's cost
...
The gap widens with repo size and flow complexity: on medium/large repos the without-lattice-sensor
arm often **thrashes** — many greps/globs, shell `find`/`grep` (Bash), and occasionally spawning
a **sub-agent** — while the with-lattice-sensor arm answers in 2–8 calls. On tiny repos (a handful of
files) the two arms tie or lattice-sensor is marginally slower (MCP/index overhead doesn't pay off
...
- **cg-calls** = lattice-sensor MCP calls in the "with" arm (the trade for reads/greps).
...
**Totals (37 cells):** with lattice-sensor **38 reads / 22 greps**, without **159 reads / 72 greps** —
**76% fewer reads, ~69% fewer greps.** Lattice Sensor never increased reads in any cell, and the
...
- **Without lattice-sensor, large repos make the agent thrash:** it falls back to shell `find`/`grep`
  (37 Bash calls across the matrix) and on jellyfin even spawned a sub-agent — exactly the behavior
  lattice-sensor is meant to prevent. The with-arm answers those in 2–8 lattice-sensor calls and used **0 Bash
  and 0 sub-agents** anywhere.
- **Tie zone = tiny repos** (Kotlin Jetcaster 1R/1R, Rust cratesio 1R/1R, express 1R/2R, Swift template
  0R/2R): the whole flow fits in 1–2 files, so reading is already cheap; lattice-sensor ties on reads and is
  sometimes a few seconds slower (MCP + index overhead — Kotlin petclinic 37s vs 23s, cratesio 22s vs
  18s). This matches the design note that lattice-sensor's value scales with repo size.
...
- Some "with" cells still read 2–4 files (jellyfin, gitness, forem, saleor, django) — the residual is
  the documented frontier (anonymous handlers, deep service chains, dynamic finders); lattice-sensor gets the
