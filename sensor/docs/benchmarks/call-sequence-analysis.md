...
The [A/B matrix](lattice-sensor-ab-matrix.md) showed lattice-sensor cuts **reads 75%** but **wall-clock only
...
4. **Round-trips are 25% fewer with lattice-sensor (283 vs 375 turns)** but wall-clock is only 16%
   faster — because the with-arm's turns each carry a ~18K explore payload, inflating TTFT and
   eroding the turn savings.
5. **Root cause:** `src/mcp/server-instructions.ts` leads with *"answer directly … `lattice-sensor_context`
   first, then ONE `lattice-sensor_explore`"* as the headline pattern. The trace-first guidance is buried
...
| cells that called `lattice-sensor_trace` | **3** (`cpp-leveldb`, `excalidraw`, `c-redis`) |
...
Across all cells, per lattice-sensor tool — call count and **average payload per call**:
...
With-arm **total** lattice-sensor payload by repo-size tier:
...
in this harness), a fixed 1-turn tax before any lattice-sensor call. Worth confirming whether the
production install defers lattice-sensor tools the same way.
...
- **Change:** rewrite the `server-instructions.ts` headline so a *flow* question (how does X reach Y
  / trace / from→to) routes to `lattice-sensor_trace` **first**, demoting the context→explore pattern to
  non-flow/onboarding questions. Mirror into `instructions-template.ts` + `.cursor/rules/lattice-sensor.mdc`.
...
`LATTICE_SENSOR_MCP_TOOLS` allowlist (so an ablated tool is genuinely absent from ListTools, not
...
2. **Strengthen the steering.** Arm A (shipped server-instructions, which *already* say "trace first
   ...
   `instructions-template.ts` + `.cursor/rules/lattice-sensor.mdc` (house rule: all three together),
...
- **SHIP: the `LATTICE_SENSOR_MCP_TOOLS` allowlist** — independent, clean, validated.
...
- **SHIP: the `LATTICE_SENSOR_MCP_TOOLS` allowlist** — independent, validated.
...
same queries, **median of 4 runs/arm** (headless: lattice-sensor-only MCP vs empty MCP):
...
cache-ability**: lattice-sensor answers in far fewer turns over a much smaller accumulated context, while
