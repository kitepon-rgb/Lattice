...
**Question:** Does answering a "how does X work?" question *directly* with Lattice Sensor in the
...
**Short answer:** No. With Lattice Sensor, main-session context is roughly **scale-invariant (~50k)**
...
- **Arms:** `WITH` = Lattice Sensor in the MCP config; `WITHOUT` = empty MCP config (`--strict-mcp-config`).
...
| metric | WITH lattice-sensor | WITHOUT |
|---|---|---|
...
| lattice-sensor calls | ~8 (search + explore×2–3 + context) | 0 |
| Reads (main+sub) | 0 / 1 / 0 | 6 / 26 / 19 |
| billable tokens | ~126k | ~176k |

## Findings

**Main-session context is scale-invariant with Lattice Sensor.** With lattice-sensor, main-session
context was **~47k on VS Code — essentially identical to Excalidraw's ~50k**, despite a 16×
bigger repo. It didn't balloon. Reason: lattice-sensor's `explore` payload is **budget-capped** and
retrieval is **targeted** — answering one question pulls in the relevant *flow/area*, not more
just because the repo is huge. So lattice-sensor makes main-session context roughly scale-invariant
...
Claude Code avoids that **without** lattice-sensor by delegating to an Explore agent (29–31k main),
but at the cost of **17–26 reads** and ~28% more tokens. Lattice Sensor keeps main lean a *better*
way: a capped, targeted payload — no delegation, **0 reads**.

**On "the Explore agents use lattice-sensor."** I couldn't reproduce it: across **6/6**
with-lattice-sensor runs (both repos), Claude Code **never delegated** — it answered directly every
time. The Explore-agent path only appeared in the `without` arm (using grep/read, since lattice-sensor
wasn't in that config). So with the current instructions + lattice-sensor present, Claude Code stays
in the main session — the lean-main-via-Explore-agent best case simply isn't what happens;
lean-main-via-capped-lattice-sensor is, and it's cheaper.

## Verdict

**"Answer directly with lattice-sensor" wins for Claude Code too — at every scale.** No per-agent
...
directly). This conclusion drove updating the README's `## Lattice Sensor` example block, which
previously told agents to "NEVER call `lattice-sensor_explore` directly / ALWAYS spawn an Explore
agent" — i.e., it steered Claude Code toward the *worse* (17–26 read, ~28%-more-token) path.

**Caveat / future work (not a blocker):** an Explore agent that *itself uses lattice-sensor* could in
