...
**Status: PORT COMPLETE (2026-07-20)** — walker `lattice-sensor-kernel/src/csharp.rs`,
...
row; suite ×2 under LATTICE_SENSOR_KERNEL_EXPECT=1; DEFAULT_ROUTED += csharp). This
...
(`/private/tmp/claude-501/-Users-colby-Development-LatticeSensor-lattice-sensor/0c11bda1-0b19-4fec-bcd9-d0cb4b2d6e8a/scratchpad/svy-csharp/`:
...
- **Cargo plan:** add `tree-sitter-c-sharp = "=0.23.5"` to
  `lattice-sensor-kernel/Cargo.toml` following the exact-pin comment block (the
  ...
  `lattice-sensor-kernel/src/langs.rs` + add `"csharp"` to `LANGUAGES`. The crate's
...
4. **One walker module** (suggest `lattice-sensor-kernel/src/csharp.rs`), registered
...
are csharp's minus/plus the declarator switch): `LATTICE_SENSOR_VALUE_REFS=0` kill;
...
- **Parity sweeps** (`scripts/kernel-parity.mjs`, ORDER-sensitive
  ...
  `/private/tmp/claude-501/-Users-colby-Development-LatticeSensor-lattice-sensor/765a9532-0a92-43de-8d50-7c8ca1cb345c/scratchpad/{serilog,Newtonsoft.Json,jellyfin}`
  ...
  **full-init dump-diffs byte-identical** (kernel arm vs `LATTICE_SENSOR_KERNEL=0`,
...
- **Suite**: new `__tests__/kernel-csharp-parity.test.ts` (torture + CRLF +
  defer fixtures, strict full-object compare, `LATTICE_SENSOR_KERNEL_EXPECT=1`
