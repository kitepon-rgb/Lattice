...
**Status: PORT COMPLETE (2026-07-20)** — walker `lattice-sensor-kernel/src/kotlin.rs`
+ the vendored-grammar-C build (lattice-sensor-kernel/grammars/kotlin via build.rs
...
sha-matched 0.3.8 `parser.c`+`scanner.c` inside `lattice-sensor-kernel` via
...
- **Kernel side — the NOVEL part (crate pin impossible):** the 0.3.8 crate's
  ...
  - copy the tag's `src/parser.c`, `src/scanner.c`, and `src/tree_sitter/*.h`
    to `lattice-sensor-kernel/grammars/kotlin/` (shas above, recorded in a comment);
  - `lattice-sensor-kernel/build.rs`: `cc::Build` compiling both C files with the
...
- **Staging plan (bump PR, before any walker exists):** vendor the wasm to
  ...
  "crate unusable — kernel compiles vendored C, see lattice-sensor-kernel/grammars/
...
4. **One walker module** (suggest `lattice-sensor-kernel/src/kotlin.rs`; no crate
...
Port the full machinery (crib java.rs/go.rs): `LATTICE_SENSOR_VALUE_REFS=0` kill;
...
- **Parity sweeps** (`scripts/kernel-parity.mjs <dir>`, order-sensitive
  ...
  - `/private/tmp/claude-501/-Users-colby-Development-LatticeSensor-lattice-sensor/0c11bda1-0b19-4fec-bcd9-d0cb4b2d6e8a/scratchpad/gate-repos/okio` (small, 322 kt/kts files)
  ...
  `LATTICE_SENSOR_KERNEL=0`, `dump-graph.mjs`, cmp) on the same three.
...
- **Suite**: kernel-kotlin-parity torture + CRLF variants + the defer
  ...
  suite ×2 green with `LATTICE_SENSOR_KERNEL_EXPECT=1`.
