...
**Status: PORT COMPLETE (2026-07-20)** — walker `lattice-sensor-kernel/src/rustlang.rs`,
...
4. **One walker module** (suggest `lattice-sensor-kernel/src/rustlang.rs` — "rust"
...
Port the full machinery (crib go.rs/tsjs): `LATTICE_SENSOR_VALUE_REFS=0` kill;
...
- **Parity sweeps** (`scripts/kernel-parity.mjs`, order-sensitive full-object):
  **ripgrep (small), tokio (medium), rust-analyzer (large)** — all three also
  ...
  dump-diffs byte-identical** (kernel arm vs `LATTICE_SENSOR_KERNEL=0`,
...
- Suite green with `LATTICE_SENSOR_KERNEL_EXPECT=1`; unit tests for the walker in
