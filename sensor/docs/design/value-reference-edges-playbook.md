...
**Where it flows:** straight into `getImpactRadius` → `lattice-sensor impact` and the impact trail
in `lattice-sensor_explore` / `lattice-sensor_node`. No agent-behaviour change required. **The win is
...
| `valueRefsEnabled` | `process.env.LATTICE_SENSOR_VALUE_REFS !== '0'` — default ON, env opts out. |
...
- **Default ON** for TS/JS/tsx + Go + Python + Rust + Ruby + C + Java + C# (`LATTICE_SENSOR_VALUE_REFS=0` disables). Shipped in **PR #895**
...
- **Tests:** `__tests__/value-reference-edges.test.ts` — same-file readers edged; surfaced in
  ...
  edged (tsx); `LATTICE_SENSOR_VALUE_REFS=0` emits nothing.
...
Index the same repo twice (on vs `LATTICE_SENSOR_VALUE_REFS=0`); node count **must be identical**
...
CG="$(pwd)/dist/bin/lattice-sensor.js"
export LATTICE_SENSOR_TELEMETRY=0 DO_NOT_TRACK=1 LATTICE_SENSOR_NO_DAEMON=1
...
OND="$ON/.lattice/sensor/sensor.db"; OFD="$OFF/.lattice/sensor/sensor.db"
...
for s in SOME_CONST ANOTHER_CONST; do
  printf "%-20s ON %s OFF %s\n" "$s" \
    "$(node dist/bin/lattice-sensor.js impact "$s" --path "$ON"  2>/dev/null | grep -oE '— [0-9]+ affected' | head -1)" \
    "$(node dist/bin/lattice-sensor.js impact "$s" --path "$OFF" 2>/dev/null | grep -oE '— [0-9]+ affected' | head -1)"
done
```
...
`lattice-sensor_search`/`callers`, *not* `impact`/`explore`, so it often doesn't query the
value-ref edges at all. ON was never worse than OFF. **So: value-refs does NOT reduce agent
reads — the win is blast-radius correctness** (impact API / Lattice Sensor Pro's verdict engine).
...
- **`Lattice Sensor.initSync(dir, opts)` ignores `opts`** — it takes only the path; the default
