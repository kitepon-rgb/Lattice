...
module (`lattice-sensor-kernel/src/lua.rs`, ccpp-style language-parameterized —
...
- **lua** — vendored wasm ≡ **tree-sitter-grammars/tree-sitter-lua v0.4.1**,
  ...
  **ABI 15, STATE_COUNT 262, SYMBOL_COUNT 137, FIELD_COUNT 22** (verified in
  ...
  mechanism (`lattice-sensor-kernel/grammars/kotlin` + build.rs cc precedent).
...
- **luau** — vendored wasm ≡ **tree-sitter-grammars/tree-sitter-luau v1.2.0
  ...
  **ABI 14, STATE_COUNT 585, SYMBOL_COUNT 197, FIELD_COUNT 21**. Crate deps:
  ...
  Route: **`tree-sitter-luau = "=1.2.0"`** in lattice-sensor-kernel/Cargo.toml,
  `"luau" => Some(tree_sitter_luau::LANGUAGE.into())` in langs.rs.
- **Lua kernel C vendor** (`lattice-sensor-kernel/grammars/lua/`), from the v0.4.1
...
1. **One walker module, two grammar entries.** `lattice-sensor-kernel/src/lua.rs`
...
4. **`.lua` → `lua`, `.luau` → `luau`** purely by extension
   ...
   `lattice-sensor.json` extension overrides are TS-side and upstream of the
...
- **Torture fixtures** per §Fixtures below, in a new
  ...
  CRLF variants; full suite ×2 green with `LATTICE_SENSOR_KERNEL_EXPECT=1`.
- **Parity sweeps** (`scripts/kernel-parity.mjs <dir>`, default
  ...
  `LATTICE_SENSOR_KERNEL=0`, `scripts/dump-graph.mjs`, cmp) on kong + lazy.nvim +
  lune + Fusion.
