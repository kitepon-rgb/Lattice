...
**Status: PORT COMPLETE (2026-07-20)** — walker `lattice-sensor-kernel/src/ruby.rs`,
...
- **Staging plan (grammar-bump PR, before any walker exists):** vendor the
  ...
  lattice-sensor-kernel/Cargo.toml under the exact-pin comment block (`=` like
...
4. **One walker module** (suggest `lattice-sensor-kernel/src/rubylang.rs` or
...
`LATTICE_SENSOR_VALUE_REFS=0` kill; MAX_VALUE_REF_NODES=20_000 caps prune scan and
