# Anonymous usage telemetry — 旧上流設計（未使用）

> Latticeはこの設計を運用しない。現行runtimeはtelemetryを外部送信せず、collectorもdeployしない。
> 現行契約は[`../../TELEMETRY.md`](../../TELEMETRY.md)を正とする。

Status: implemented — ingest Worker (`telemetry-worker/`), client (`src/telemetry/`),
`lattice-sensor telemetry` CLI, MCP + installer wiring, `TELEMETRY.md`. Pending: Worker deploy
+ DNS, release.
Scope: public `lattice-sensor` engine (CLI + MCP server + installer)

Lattice Sensor is a local-first tool whose whole pitch is "your code never leaves your machine."
...
- How many machines actively use lattice-sensor (daily/weekly), and how does that change?
...
- No telemetry from the `lattice-sensor-pro` fork (see "lattice-sensor-pro rule" below).
...
4. **First-party endpoint.** Clients only ever talk to `telemetry.getlattice-sensor.com`. The URL
...
| `lattice-sensor_version` | `0.9.12` | from package.json |
...
- **`usage_rollup`** — the workhorse. One event per `(day, kind, name)` per machine,
  ...
  (e.g. `lattice-sensor_explore`, `affected`), `count`, `error_count`, and for MCP:
  ...
  whose `lattice-sensor_explore` errored or returned nothing records
...
- **Headless paths** (`npx lattice-sensor init`, MCP server — no TTY, never prompt): right
...
- **Flush** — many CLI actions end via `process.exit()`, where `beforeExit` never fires
  ...
  `https://telemetry.getlattice-sensor.com/v1/events` with `AbortSignal.timeout(1500)`,
  ...
  sender's claim merges back after an hour). `LATTICE_SENSOR_TELEMETRY_DEBUG=1` echoes
...
`telemetry.getlattice-sensor.com` → small Worker living at `telemetry-worker/` in this repo —
...
## lattice-sensor-pro rule (do not lose this in upstream merges)

The private `lattice-sensor-pro` fork ships inside customer containers whose guarantee is
...
container sets `LATTICE_SENSOR_TELEMETRY=0` as belt-and-braces. This rule lives in the fork's
...
2. Worker + DNS live first (so the first shipping client never 404s), PostHog dashboards:
   ...
   languages indexed.
3. Client module + config + `lattice-sensor telemetry` subcommand + MCP `clientInfo` plumbing.
