...
- **Production wasm** sha256
  ...
  **Vendor plan: BYTE-COPY this file** to
  ...
  C — lattice-sensor-kernel/grammars/dart"). No rebuild, no `tree-sitter
...
- **Kernel side — vendored-grammar-C (the kotlin #1382 mechanism, second
  ...
  to `lattice-sensor-kernel/grammars/dart/`; extend `lattice-sensor-kernel/build.rs`'s
...
2. **No dart framework resolver exists** (grep src/resolution/frameworks/ —
   ...
   check.
3. **One walker module** (suggest `lattice-sensor-kernel/src/dart.rs`), registered
...
Port the full machinery (crib java.rs/kotlin.rs): `LATTICE_SENSOR_VALUE_REFS=0`
...
- **Parity sweeps** (`scripts/kernel-parity.mjs <dir>`, order-sensitive
  ...
  vs `LATTICE_SENSOR_KERNEL=0`, `scripts/dump-graph.mjs`, cmp) on the same
  three.
