# Design + status: adaptive `lattice-sensor_explore` sizing (sibling skeletonization)
...
"Refinement" below). Escape hatch: `LATTICE_SENSOR_ADAPTIVE_EXPLORE=0`.
**Motivation:** make `lattice-sensor_explore` size its output to the *answer* rather
...
`lattice-sensor_explore` returned full source for **every** relevant file up to its
...
the rest of the session, the WITH-Lattice Sensor arm cost *more* than the WITHOUT arm
...
A file is skeletonized iff **all** hold (and `LATTICE_SENSOR_ADAPTIVE_EXPLORE != 0`):
...
Headless `claude -p`, Opus 4.8, **WITH vs WITHOUT** Lattice Sensor (the real benchmark
