...
> **Update (2026-06-01):** the `lattice-sensor_trace` and `lattice-sensor_context` MCP tools
> were since **removed** — `lattice-sensor_explore` is the single surfacing tool now. Its
> "Flow" section (`buildFlowFromNamedSymbols`) and the `lattice-sensor_node` trail surface
> these synthesized edges; the `trace(a, b)` notation below means "the a→b flow,"
> which you now verify with `lattice-sensor_explore` / `probe-explore.mjs` (the
...
sqlite3 /tmp/lattice-sensor-corpus/excalidraw/.lattice/sensor/sensor.db \
...
node scripts/agent-eval/probe-explore.mjs /tmp/lattice-sensor-corpus/excalidraw "triggerUpdate triggerRender"
```
...
- **Retrieval/UX changes** (separate from coverage): `explore` whole-small-file + glue
  ...
  — all in `src/mcp/tools.ts`. (`lattice-sensor_trace` / `lattice-sensor_context` were later
  removed; explore is the one surfacing tool.)
- **Full investigation context + findings:** auto-memory
  `project_lattice-sensor_read_displacement` (why coverage — not prompting/hooks/new-tools —
  is the lever for getting agents to use lattice-sensor over Read).
