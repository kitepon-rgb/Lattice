...
**Mission:** make lattice-sensor's `trace` / `callers` / `callees` / `impact` /
...
After #165, lattice-sensor indexes Swift, Objective-C, and JavaScript/TypeScript
...
3. **Agent A/B (with vs without lattice-sensor, ≥2 runs/arm):** Read = 0 within the explore-call budget; faster than without-lattice-sensor; no regression on a pure-Swift or pure-ObjC control repo (e.g. Texture).
...
5. **Agent A/B** as above. Critical: a question that *crosses the bridge* (e.g. "how does pressing Button X reach the network call") must drop Read to 0 in ≥1 run with lattice-sensor.
...
The resolver's `initialize()` runs at Lattice Sensor construction — before any
...
- All three §5a corpora pass: pure-language probes unchanged; cross-language
  ...
  in ≥1 run with lattice-sensor, faster than without.
...
actively make lattice-sensor worse on these codebases than not having any
bridging at all.
