...
dispatch** across **every language and framework lattice-sensor supports**, and validate
...
> Full investigation context + findings: auto-memory `project_lattice-sensor_read_displacement`.

> **Update (2026-06-01):** the `lattice-sensor_trace` and `lattice-sensor_context` MCP tools were
> **removed** — `lattice-sensor_explore` is the single surfacing tool now. Its "Flow" section
> (`buildFlowFromNamedSymbols`) surfaces the synthesized edges this playbook is about, and
> you validate coverage with `lattice-sensor_explore` / `scripts/agent-eval/probe-explore.mjs`.
...
lattice-sensor's value is being **the map** — answering structural/flow questions
(`trace`, `impact`, callers, "how does X reach Y") that grep/Read cannot. Agents
will use lattice-sensor instead of Read **only when it is sufficient**. We proved
...
- **Fix:** `closureCollectionEdges` (callback-synthesizer.ts). A **dispatcher** iterates a collection
  *invoking each element* (`coll.forEach { $0() }` / `{ it() }`); a **registrar** appends a closure to
  ...
  symbols" section in `lattice-sensor_explore` (`buildFlowFromNamedSymbols`) so the relationship shows even
...
- **Result:** `trace(didCompleteTask, validate)` connects with the closure-collection hop + the
  ...
  Forced lattice-sensor-only (Read+Grep+Bash blocked): 3/3 runs answer build/send/validate correctly.
...
`lattice-sensor_trace`'s endpoint disambiguation (`scorePair`, shared-dir-prefix only) was resolving an
...
a fixable bug. The transcript reasoning, not the median, tells you which. The forced lattice-sensor-only
...
4. **End-to-end agent eval:** run the flow question with lattice-sensor and measure
   **reads / answer-completeness / cost** vs a pre-fix baseline:
   ...
   ```
   Then parse: `Read` count, lattice-sensor-tool count, cost, and whether the answer now
...
- **Coverage enables, doesn't force, the no-read path.** Agents still read to *confirm
  source* sometimes; cost stays ~flat (lattice-sensor calls trade for reads). The reliable
...
- **Svelte / SvelteKit (validated 2026-05-23, realworld S / skeleton M / shadcn L) — already well-covered.**
  ...
  lattice-sensor **1 read** vs without **4** — lattice-sensor already wins out of the box. The one extraction gap
...
- **Express / Koa (validated 2026-05-23, realworld S / parse M / ghost L) — high-value inline-handler fix.**
  ...
  by repo characteristics** — realworld (39 files) is below the size where lattice-sensor beats reading, and
...
- **NestJS (validated 2026-05-23, realworld S / immich M-L / amplication L) — already well-covered.** The
  ...
  name + co-location, no type_of edge needed. Agent A/B (immich album flow): lattice-sensor **eliminated Grep
  (0 vs 3)** tracing route→controller→service. No dynamic-dispatch hole. One GENERAL hygiene gap surfaced
  (not NestJS-specific): the realworld example **commits its `dist/`** build output, which lattice-sensor indexes
...
- **Rails (validated 2026-05-23, realworld S / spree M / forem L) — high-value RESTful-routing fix.** The
  ...
  **0→635** precise route→action edges. Agent A/B (forem comment-creation, large): lattice-sensor **1–4 reads /
...
- **Spring (validated 2026-05-23, realworld S / mall M / halo L) — bare-mapping + class-prefix routing fix.**
  ...
  resolves (`article→findBySlug`). Agent A/B (mall cart flow): with lattice-sensor 0 reads/0 grep vs without 2/2.
...
- **Django / DRF (validated 2026-05-23, realworld S / wagtail M / saleor L) — mostly covered + a DRF-router
  ...
  Agent A/B (wagtail Page flow, medium): lattice-sensor **4–7 reads / 1–4 grep / 58–81s** vs without **7–9 reads
...
- **Laravel (validated 2026-05-23, realworld S / firefly M / bookstack L) — route precision fix.** The
  ...
  PageApiController.list). Agent A/B (bookstack page-view, large): lattice-sensor **2–3 reads / 1–2 grep /
...
- **Gin / chi (validated 2026-05-23, realworld S / gin-vue-admin M / gitness L) — group-var routing fix.**
  ...
  CreateInfo`); realworld stable (no regression); no garbage. **Agent A/B (create-user flow): lattice-sensor
...
- **ASP.NET Core (validated 2026-05-23, realworld S / eShopOnWeb M / jellyfin L) — detection + bare-attribute
  ...
  Agent A/B (eShop catalog listing): lattice-sensor **1–2 reads / 0 grep / 63–75s** vs without **6–7 / 1–6 /
...
- **Flask / FastAPI (validated 2026-05-23, fastapi-realworld S / flask-microblog S / Netflix dispatch L /
  ...
  `create_user → from_dict`). Agent A/B (realworld login-auth flow, n=2/arm): lattice-sensor **0–1 read / 0 grep /
  3–4 lattice-sensor / 30–39s** (context→[search]→trace→node) vs without **3 read / 2 grep / 33–36s** — eliminates
...
- **Drupal (validated 2026-05-23, admin_toolbar S / webform M / drupal-core L) — pre-filter + detection fixes.**
  ...
  n=2/arm): lattice-sensor **0 read / 1 grep / 20–22s** vs without **1 read / 2 grep + glob / 28–32s** — fewer
...
- **Rust / Axum + Rocket + actix (validated 2026-05-23, realworld-axum S / actix-examples + Rocket M / crates.io L) — Axum chained-method + namespaced-handler fix.**
  ...
  n=2/arm): lattice-sensor **0–2 read / 0 grep / 32–40s** vs without **3 read / 0–1 grep + glob / 33–41s** — modest
...
- **Swift / Vapor (validated 2026-05-23, vapor-template S / SteamPress M / SwiftPackageIndex-Server L) — the resolver was effectively dead on real apps.**
  ...
  all — the Vapor ecosystem's routing styles vary widely. Agent A/B (create-post flow, n=2/arm): lattice-sensor
  **0 read / 0 grep / 4 lattice-sensor / 26–30s** (both runs fully clean) vs without **1–4 read / 0–2 grep +
...
- **Kotlin / Spring Boot + Jetpack Compose (validated 2026-05-23, spring-petclinic-kotlin S / compose-samples) — extend Spring to Kotlin; Compose is free.**
  ...
  **0→18, 18/18**; class `@RequestMapping` prefixes join, stacked annotations (`@ResponseBody`) are skipped, DI
  ...
  constructors. Agent A/B (view-owner flow, n=2/arm): lattice-sensor **0–1 read / 0 grep / 1 lattice-sensor / 11–18s** (a
...
- **Lua / Luau (validated 2026-05-23, telescope.nvim / lualine.nvim / Knit — measure-first, already covered).**
  ...
  plugins are MODULE-dispatch-heavy (`local m = require('telescope.actions'); m.fn()`), and lattice-sensor's general
  ...
  Agent A/B (actions.utils map flow, n=2/arm): lattice-sensor **0 read / 0 grep / 18–24s** vs without **1 read
...
- **Scala / Play (validated 2026-05-23, play-samples: computer-database / starter / rest-api) — Play conf/routes → controller.**
  ...
  connects request→route→controller→DAO. A/B (list-computers, n=2/arm): lattice-sensor **0 read / 0 grep / 3
  lattice-sensor / 17–22s** vs without **2–3 read / 1–2 grep + glob / 16–17s**. **No-regression:** the file-walk
