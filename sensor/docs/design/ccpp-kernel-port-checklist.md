...
(2026-07-17).** Walker: `lattice-sensor-kernel/src/ccpp/mod.rs` (one dual-language
...
- **Deferral-rate guard — CORRECTED BY MEASUREMENT** (the §4f pattern): the
  ...
  the defer disabled (`LATTICE_SENSOR_KERNEL_CCPP_ERROR_EXTRACT=1`, sweep-only
...
3. **One walker module, dual language** (`lattice-sensor-kernel/src/ccpp/`), flagged
...
| — | value-reference edges | **c: YES** (VALUE_REF_LANGS line 401), **cpp: NO**. Port the value-ref machinery for C only (crib go.rs / tsjs — shadow prune, scope stack, MAX_VALUE_REF_NODES cap, LATTICE_SENSOR_VALUE_REFS=0 kill) |
