...
- **Vendored-C route (kotlin mechanism, second use)** — copy from the
  `0aca5d0a6f` clone into `lattice-sensor-kernel/grammars/scala/` (shas recorded in
...
3. **One walker module** (`lattice-sensor-kernel/src/scala.rs`), registered in
...
Port the full machinery (crib kotlin.rs): `LATTICE_SENSOR_VALUE_REFS=0` kill;
...
- **Suite**: kernel-scala-parity torture + CRLF variants + `.sc` fixture +
  defer fixtures (§Fixtures 5-6) ×2 green with `LATTICE_SENSOR_KERNEL_EXPECT=1`.
