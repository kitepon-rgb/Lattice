...
- [x] **R7a. C/C++ port** — DONE 2026-07-17, same-day walker+gates after the
      ...
      (`lattice-sensor-kernel/src/ccpp/`), preParse HOISTED to the route point
...
- [x] **R7b. Remaining long tail — COMPLETE 2026-07-20** (all details in the
      ...
      **rust DONE 2026-07-20** (first R7b port): grammar bumped to
      ...
      full suite green), walker `lattice-sensor-kernel/src/rustlang.rs` (survey
...
- `src/extraction/kernel/` — loader (contract-verifies before routing; a stale
  ...
  `LATTICE_SENSOR_KERNEL_LANGS` REPLACES the set; `LATTICE_SENSOR_KERNEL=0` kills), the
...
3. **The pool loses to sequential at 2 real cores** (853s vs 1,150s resolution;
   ...
   `LATTICE_SENSOR_RESOLVE_WORKERS` knob (#1333/#1335); ap=2 → sequential by choice.
...
5. **Silent failure modes burned three 25-minute cycles**: give-ups were
   ...
   nothing. All valve/sizing decisions now print under `LATTICE_SENSOR_SYNTH_TIMINGS`
   / `LATTICE_SENSOR_WAL_VALVE_DEBUG` — the armed line answers "is it even alive"
   in one glance.
...
`LATTICE_SENSOR_RESOLVE_PROFILE` (shipped in #1339: per-outcome resolveOne histogram
...
(`LATTICE_SENSOR_SYNTH_TIMINGS` prints the `cFnPtr sub:` line):
...
Fresh `LATTICE_SENSOR_RESOLVE_PROFILE` tables on the round-2 build (v7.2-rc2 tree,
...
Step 2 shipped: `cfnptr_scan_files` in the kernel (lattice-sensor-kernel/src/
...
`LATTICE_SENSOR_KERNEL=0`, or the scanner's own `LATTICE_SENSOR_KERNEL_CFNPTR=0`).
...
`prebuilds/linux-arm64/lattice-sensor-kernel.node`).
...
(`LATTICE_SENSOR_RESOLVE_PROFILE` stage tables, caffeinated, same day/build,
