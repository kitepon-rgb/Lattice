...
**Status: PORT COMPLETE (2026-07-20)** — walker `lattice-sensor-kernel/src/php.rs`,
...
- **Staging plan:** vendor to `src/extraction/wasm/tree-sitter-php.wasm`, add
  ...
  `tree-sitter-php = "=0.24.2"` in lattice-sensor-kernel/Cargo.toml (crate + wasm
  move TOGETHER), add `'php'` to `GRAMMAR_LANGUAGES` in
  `__tests__/kernel-grammar-parity.test.ts:39` and to `grammar_for` in
  `lattice-sensor-kernel/src/langs.rs` (+ the `LANGUAGES` const). MIT license, same
...
4. **One walker module** (suggest `lattice-sensor-kernel/src/php.rs`), registered in
...
Port the full machinery (crib go.rs/java.rs): `LATTICE_SENSOR_VALUE_REFS=0` kill;
...
- **Parity sweeps** (`scripts/kernel-parity.mjs <dir>`, order-sensitive
  ...
  - `/private/tmp/claude-501/-Users-colby-Development-LatticeSensor-lattice-sensor/765a9532-0a92-43de-8d50-7c8ca1cb345c/scratchpad/monolog` (small, 217 files)
  ...
  byte-identical** (kernel arm vs `LATTICE_SENSOR_KERNEL=0`, `dump-graph.mjs`,
...
- **Suite**: new `__tests__/kernel-php-parity.test.ts` — torture + CRLF
  ...
  wasm output is served; full suite ×2 green with `LATTICE_SENSOR_KERNEL_EXPECT=1`.
