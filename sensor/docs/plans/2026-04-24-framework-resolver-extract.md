...
Rationale for splitting the two test files: the unit tests are deterministic string-in / array-out and run in milliseconds; the integration test boots a Lattice Sensor DB and is slower but gives the strongest behavioral guarantee.
...
import { Lattice Sensor } from '../src';
...
describe('Django end-to-end', () => {
  ...
  it('creates a route->view edge from urls.py to view class', async () => {
    ...
    const cg = new Lattice Sensor(tmpDir);
    await cg.initialize();
    await cg.indexAll();
...
Lattice Sensor recognizes web framework routing files and links URL patterns to their handlers:
...
Query `lattice-sensor_callers(YourView)` and the route pattern will appear as an incoming edge.
```
...
```bash
gh pr create \
  --repo colbymchenry/lattice-sensor \
  --base main \
...
`FrameworkResolver.extractNodes` is declared in the type but never called anywhere in `src/`. As a result, the graph has zero `route` nodes for any framework, and the URL-to-handler link (e.g. Django `urls.py` entry -> view class) doesn't exist. This makes `lattice-sensor_callers(MyView)` silently miss its most important caller.
